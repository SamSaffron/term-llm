...
function deterministicStyles(): Plugin {
  return {
    ...
    transform(code, id) {
      ...
      return { code: code.replace(/src:url\(([^)]+\.woff2)\) format\("woff2"\),url\([^)]+\.woff\) format\("woff"\),url\([^)]+\.ttf\) format\("truetype"\)/g, 'src:url($1) format("woff2")'), map: null };
    },
    generateBundle(_options, bundle) {
      ...
      for (const output of Object.values(bundle)) {
        ...
        const target = css.length > 100_000 ? 'app.css' : css.includes('KaTeX_Main') ? 'chunks/katex.css' : css.includes('.hljs') ? 'chunks/highlight.css' : `assets/${output.fileName.split('/').pop()}`;
    ...
    rollupOptions: {
      ...
      output: {
        ...
        manualChunks(id) {
          ...
          if (id.includes('/node_modules/preact/') || id.includes('/node_modules/@preact/signals/') || id.includes('/node_modules/marked/') || id.includes('/node_modules/dompurify/')) return 'vendor';
          return undefined;
        },
      },
    },
