...
export class APIError extends Error {
  ...
  constructor(message: string, readonly status: number, readonly body = '', type = '') {
    super(message);
    this.name = 'APIError';
    this.type = type;
  }
...
export function classifyRequest(init: RequestInit = {}, controls: RequestControls = {}): RequestClassification {
  const method = String(init.method || 'GET').toUpperCase();
  let policy = controls.policy || (SAFE_METHODS.has(method) ? 'safe-read' : 'mutation');
  const headers = new Headers(init.headers);
  if (policy === 'idempotent-mutation' && method === 'POST' && !headers.has('Idempotency-Key')) policy = 'mutation';
  ...
  return {
    ...
    timeoutMs: controls.timeoutMs === 0 ? 0 : Math.max(1, controls.timeoutMs ?? (SAFE_METHODS.has(method) ? DEFAULT_TIMEOUT_MS : DEFAULT_MUTATION_TIMEOUT_MS)),
  };
}

export function trustedExternalLoginURL(response: Pick<Response, 'status' | 'headers'>, currentURL = location.href): string {
...
function retryableStatus(status: number): boolean { return RETRYABLE_STATUSES.has(status) || status >= 500; }
function abortError(error: unknown): boolean { return error instanceof DOMException ? error.name === 'AbortError' : (error as { name?: string } | null)?.name === 'AbortError'; }
...
export class APIClient {
  constructor(readonly config: AppConfig, private hooks: TransportHooks) {}
    ...
    for (let attempt = 0; attempt <= classification.retries; attempt += 1) {
      ...
      try {
        ...
        if (response.status === 401 && controls.auth === 'session' && !externalAuthRedirecting) {
          ...
          if (loginURL) { externalAuthRedirecting = true; location.assign(loginURL); }
        }
        if ((response.status === 401 || response.status === 403) && controls.auth !== 'ignore' && !externalAuthRedirecting) this.hooks.onAuthRequired();
        if (retryableStatus(response.status) && classification.retryable && attempt < classification.retries) {
          ...
          try { await response.body?.cancel(); } catch { /* A replacement request owns recovery. */ }
          timed.cleanup();
        ...
        this.hooks.onNetworkState?.(navigator.onLine === false ? 'offline' : 'retrying', String(error));
  ...
  async json<T>(path: string, init: RequestInit = {}, policyOrControls?: FetchPolicy | RequestControls): Promise<T> {
    ...
    if (!response.ok) {
      ...
      let type = '';
      try {
        const parsed = JSON.parse(body) as { error?: string | { message?: string; type?: string; code?: string }; message?: string; type?: string; code?: string };
        if (typeof parsed.error === 'object') { message = parsed.error.message || message; type = parsed.error.type || parsed.error.code || ''; }
        else { message = parsed.error || parsed.message || message; type = parsed.type || parsed.code || ''; }
  ...
  get<T>(path: string, signal?: AbortSignal, controls?: Omit<RequestControls, 'policy'>): Promise<T> { return this.json<T>(path, { signal }, { policy: 'safe-read', ...controls }); }
  post<T>(path: string, body: unknown, policy: FetchPolicy = 'mutation', headers?: HeadersInit): Promise<T> {
    return this.json<T>(path, { method: 'POST', headers, body: body instanceof FormData ? body : JSON.stringify(body) }, policy);
  }
  patch<T>(path: string, body: unknown): Promise<T> { return this.json<T>(path, { method: 'PATCH', body: JSON.stringify(body) }, 'idempotent-mutation'); }
  delete<T>(path: string, body?: unknown): Promise<T> { return this.json<T>(path, { method: 'DELETE', body: body === undefined ? undefined : JSON.stringify(body) }, 'idempotent-mutation'); }
}

export interface SSEMessage { event: string; data: string; id?: string }
export async function* decodeSSE(body: ReadableStream<Uint8Array>, signal?: AbortSignal, onActivity?: () => void): AsyncGenerator<SSEMessage> {
  ...
  try {
    while (!signal?.aborted) {
      ...
      while ((boundary = buffer.search(/\r?\n\r?\n/)) >= 0) {
        ...
        for (const line of block.split(/\r?\n/)) {
          if (line.startsWith(':')) { onActivity?.(); continue; }
    ...
    try { await reader.cancel(signal?.reason); } catch { /* Reader is already closed. */ }
    reader.releaseLock();
  }
}
