...
class ErrorBoundary extends Component<{ children: ComponentChildren }, { error: string }> {
  state = { error: '' };
  componentDidCatch(error: Error) { this.setState({ error: error.message || 'The chat UI encountered an error.' }); }
  render(props: { children: ComponentChildren }, state: { error: string }) {
    return state.error ? <div class="startup-splash" role="alert"><div class="startup-card"><div class="startup-title">The chat UI could not continue</div><div class="startup-subtitle">{state.error}</div><button class="btn primary" onClick={() => location.reload()}>Reload</button></div></div> : props.children;
  }
}
...
export function App({ store }: { store: AppStore }) {
  ...
  useEffect(() => {
    void store.bootstrap(); void registerServiceWorker(store.config);
    const shortcut = (event: KeyboardEvent) => {
      const mac = /Mac|iPhone|iPad|iPod/.test(navigator.platform); const primary = mac ? event.metaKey && !event.ctrlKey : event.ctrlKey && !event.metaKey;
      if ((event.key === 'k' || event.key === 'K') && primary && !event.altKey && !event.shiftKey && store.widgets.peek().length) { event.preventDefault(); store.modal.value = store.modal.peek() === 'widgets' ? '' : 'widgets'; }
    };
    addEventListener('keydown', shortcut); return () => removeEventListener('keydown', shortcut);
  }, [store]);
  useEffect(() => {
    const brand = store.config.title.trim(); const title = session?.title || '';
  ...
  return <ErrorBoundary><StoreContext.Provider value={store}>
    ...
    <Modals />
    <Lightbox />
  </StoreContext.Provider></ErrorBoundary>;
}
