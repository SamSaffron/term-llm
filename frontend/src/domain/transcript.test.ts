...
describe('transcript domain', () => {
  ...
  it('windows old turns behind a stable gap and always keeps the tail', () => {
    const messages = Array.from({ length: 240 }, (_, index): Message => ({ id: String(index), role: index % 3 === 0 ? 'user' : 'assistant', content: String(index), created: index }));
  ...
  it('hands projected rows off atomically to matching durable identities', () => {
    const projected: Message[] = [
      ...
      { id: 'p2', role: 'assistant', content: 'answer', created: 2, responseId: 'r1', assistantSegmentOrdinal: 0 },
    ];
    const durable: Message[] = projected.map((message, index) => ({ ...message, id: `d${index}`, durableRowId: index + 1 }));
