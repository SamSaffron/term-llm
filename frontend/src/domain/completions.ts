...
export interface MentionSearchToken { start_utf16: number; end_utf16: number; query: string; quoted?: boolean }
export interface MentionSearchItem { path: string; kind: 'file' | 'directory'; insert_text: string; segments?: Array<{ text: string; matched?: boolean }> }
export interface MentionSearchResponse { active?: boolean; token?: MentionSearchToken; items?: MentionSearchItem[]; index_truncated?: boolean }

export const SLASH_COMMANDS = [
  { command: '/compact', description: 'Compact conversation context' },
  { command: '/effort', description: 'Choose reasoning effort' },
  { command: '/fork', description: 'Create a parallel continuation from the last safe point', streamingSafe: true },
  ...
  { command: '/side', description: 'Ask a side question without interrupting the main run', streamingSafe: true },
  { command: '/skills', description: 'Browse available skills' },
  { command: '/thread', description: 'Start a related conversation with optional context', streamingSafe: true },
...
export function skillCompletions(skills: SkillCommand[]): Completion[] {
  ...
  return skills.flatMap((skill): Completion[] => {
    ...
    const value = `/${bare}`;
    if (!/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(bare) || skill.collides_with_builtin || BUILTIN_NAMES.has(value) || seen.has(value)) return [];
    seen.add(value);
...
export function composerCompletions(value: string, agents: string[], skills: SkillCommand[] = [], streaming = false): Completion[] {
  const slash = value.match(/^\/[\w-]*$/);
  if (slash) {
    const entries: Completion[] = [
      ...SLASH_COMMANDS.map((entry) => ({ value: entry.command, label: entry.command, description: entry.description, kind: 'slash' as const, streamingSafe: entry.streamingSafe })),
      ...skillCompletions(skills),
    ];
    return entries.filter((entry) => entry.value.startsWith(slash[0]) && (!streaming || entry.streamingSafe)).sort((left, right) => left.value.localeCompare(right.value));
  }
  const mention = activeMentionAtCursor(value, value.length);
  if (mention) return agents.filter((agent) => agent.toLowerCase().startsWith(mention.query.toLowerCase())).map((agent) => ({ value: `@${agent}`, label: `@${agent}`, description: 'Agent', kind: 'agent', replacement: { start: mention.start, end: mention.end } }));
  return [];
}

export function activeMentionAtCursor(value: string, cursor: number): { start: number; end: number; query: string } | null {
  ...
  const at = line.lastIndexOf('@'); const token = line.slice(at);
...
export function mentionCompletions(payload: MentionSearchResponse | null): Completion[] {
  ...
  return (payload.items || []).flatMap((item): Completion[] => {
    const path = String(item.path || ''); const value = String(item.insert_text || '');
    if (!path || !value) return [];
    return [{ value, label: path, description: item.kind === 'directory' ? 'directory' : 'file', kind: 'mention', replacement, segments: item.segments || [] }];
  });
}
...
export function completedCursor(value: string, completion: Completion): number {
  ...
  return completion.replacement.start + completion.value.length + (!after || !/^\s/u.test(after) ? 1 : 0);
}
