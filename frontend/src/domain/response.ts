  ...
  const existing = [...messages]
    ...
    .find(
      (message) =>
        ...
        message.assistantSegmentOrdinal === ordinal,
    );
  if (existing) return [messages, existing, false];
  const created: Message = {
    id: `${responseId}:assistant:${ordinal}`,
    role: 'assistant',
    content: '',
    created: Date.now(),
    responseId,
    assistantSegmentOrdinal: ordinal,
  };
  return [[...messages, created], created, true];
}
function replaceMessage(messages: Message[], target: Message, patch: Partial<Message>): Message[] {
  return messages.map((message) => (message === target ? { ...message, ...patch } : message));
}
...
function appendNotice(
  ...
  event: ResponseEvent,
  content: string,
  role: Message['role'] = 'guardian-notice',
): Message[] {
  return [
    ...messages,
    {
      id: `${text(event.response_id)}:${event.type}:${event.sequence_number || Date.now()}`,
      role,
      content,
      created: Date.now(),
...
function flushPendingGuardian(
  ...
  let output = messages;
  for (const [callID, reviews] of Object.entries(pending))
    for (const review of reviews) {
      const outcome = review.outcome || 'warning';
      output = appendNotice(
        output,
        event,
        `${review.message || `Guardian ${outcome} review`} (unmatched tool call ${callID})`,
      );
    }
  return output;
}
function validate(
  ...
  if (event.type === 'response.stream_error')
    return {
      duplicate: false,
      sequence: projection.run.lastSequence,
      responseId: projection.run.responseId,
      epoch: projection.run.epoch,
    };
  const responseId = text(
    event.response_id || (event.response as Record<string, unknown> | undefined)?.id,
  );
  if (!responseId)
    throw new ResponseProtocolError('Response event is missing response_id', 'owner');
  ...
  if (!sequence)
    throw new ResponseProtocolError(
      'Response event is missing sequence_number',
      'gap',
      projection.run.lastSequence + 1,
      sequence,
    );
  if (sequence <= projection.run.lastSequence)
    return { duplicate: true, sequence, responseId, epoch };
  if (sequence !== projection.run.lastSequence + 1)
    throw new ResponseProtocolError(
      `Response event gap: expected ${projection.run.lastSequence + 1}, got ${sequence}`,
      'gap',
      projection.run.lastSequence + 1,
      sequence,
    );
...
export function reduceResponse(
  ...
  const checked = validate(projection, event);
  if (checked.duplicate) return projection;
  const responseId = checked.responseId;
  let messages = projection.messages;
  const run: ActiveRun = {
    ...projection.run,
    responseId,
    epoch: checked.epoch,
    lastSequence: checked.sequence,
  };
  ...
  switch (event.type) {
    ...
    case 'response.output_text.delta': {
      ...
      messages = replaceMessage(messages, target, {
        content: target.content + delta,
        segmentStartSequence:
          target.segmentStartSequence || number(event.segment_start_sequence, checked.sequence),
        segmentEndSequence: checked.sequence,
      });
      return {
        ...next,
        messages,
    ...
    case 'response.tool_exec.end': {
      ...
      messages = messages.map((message) =>
        message.role === 'tool-group' &&
        message.tools?.every((candidate) => candidate.status !== 'running')
          ? { ...message, status: 'done' }
          : message,
      );
      let plan = next.plan;
      ...
      if (completed?.name === 'update_plan' && !failed) {
        try {
          const value = JSON.parse(completed.arguments || '{}') as CurrentPlan;
          if (Array.isArray(value.plan)) plan = value;
        } catch {
    ...
    case 'response.guardian.review': {
      ...
      const review: GuardianReview = {
        outcome: text(event.outcome) || 'warning',
        message: text(event.message || event.text),
        model: text(event.model),
        tool: text(event.tool),
        command: text(event.command),
        path: text(event.path),
        is_write: Boolean(event.is_write),
      ...
      if (entry)
        return {
          ...next,
          messages: patchTool(messages, id, {
            guardianReviews: [...(entry.guardianReviews || []), review],
          }),
        };
      if (id)
        return {
          ...
          pendingGuardian: {
            ...projection.pendingGuardian,
            [id]: [...(projection.pendingGuardian[id] || []), review],
          },
        };
    ...
    case 'response.compaction':
      return {
        ...
        messages: [
          ...closeToolGroups(messages),
          {
            id: `${responseId}:compaction:${checked.sequence}`,
    ...
    case 'response.retry':
      return {
        ...
        retry: {
          ...
          delayMs: number(event.delay_ms),
          error: text(event.error || event.message),
        },
      };
    case 'response.attempt.discard':
      return {
        ...
        messages: messages.filter(
          (message) =>
            message.responseId !== responseId ||
            !['assistant', 'tool-group'].includes(message.role),
        ),
        pendingGuardian: {},
        phase: undefined,
        modelSwap: undefined,
    ...
    case 'response.approval.prompt':
      return {
        ...
        approval: {
          sessionId: projection.run.sessionId,
          id: text(event.approval_id || event.id),
