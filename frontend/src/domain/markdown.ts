...
marked.use({
  ...
  walkTokens(token) {
    if (token.type === 'del' && !token.raw.startsWith('~~')) {
      const mutable = token as unknown as { type: string; text: string; raw: string; tokens?: unknown };
      mutable.type = 'text'; mutable.text = mutable.raw; delete mutable.tokens;
    }
  },
});
...
export function renderMarkdown(source: string): string {
  ...
  rendered = rendered.replace(/TERM_LLM_MATH_(\d+)_TOKEN/g, (_match, index: string) => escapeHTML(math.values[Number(index)] || ''));
...
export function stableMarkdownBoundary(value: string): number { return findStableMarkdownBoundary(value, 0) || (isInCodeBlockFast(value, value.length) ? 0 : value.length); }
...
export function needsHighlight(root: HTMLElement): boolean { return Boolean(root.querySelector('pre code')); }
export function needsKatex(value: string): boolean { return /\$\$[\s\S]+?\$\$|\\\[[\s\S]+?\\\]|\\\(.+?\\\)/.test(value); }
...
export async function decorateRichContent(root: HTMLElement, source: string): Promise<void> {
  ...
  if (needsKatex(source)) {
    ...
    work.push(katexPromise.then(({ katex }) => {
      const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT); const nodes: Text[] = [];
      while (walker.nextNode()) { const node = walker.currentNode as Text; if (!node.parentElement?.closest('pre,code,textarea,script,style,.katex')) nodes.push(node); }
      for (const node of nodes) {
        let html = escapeHTML(node.data); let changed = false;
        for (const rule of MATH) html = html.replace(rule.expression, (all, expression: string) => {
          changed = true;
          try { return katex.renderToString(expression, { displayMode: rule.display, throwOnError: false, trust: false, strict: 'warn' }); } catch { return escapeHTML(all); }
        });
        if (!changed) continue;
        const span = document.createElement('span');
        span.innerHTML = DOMPurify.sanitize(html, { ADD_ATTR: ['class', 'style', 'aria-hidden'], FORBID_TAGS: ['script', 'iframe', 'object', 'embed'] });
...
export function rebaseRenderedAssetURLs(root: HTMLElement, rebase: (value: string) => string): void {
  root.querySelectorAll<HTMLImageElement>('img[src]').forEach((image) => { image.src = rebase(image.getAttribute('src') || image.src); });
