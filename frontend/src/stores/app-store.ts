...
const retryBackoff = (attempt: number): number => Math.round(Math.min(60_000, 1_000 * 1.5 ** Math.max(0, attempt)));
const worktreeError = (error: unknown): string => {
  const code = error instanceof APIError ? error.type : '';
  const messages: Record<string, string> = { project_required: 'Choose a project before selecting a worktree.', worktrees_unavailable: 'Worktrees are unavailable for this project.', project_not_found: 'This project no longer exists.', project_archived: 'Restore this project before creating a worktree.', projects_disabled: 'Worktrees are unavailable in no-project mode.' };
  return messages[code] || (error instanceof Error ? error.message : 'Worktree action failed.');
};
const array = (value: unknown): Record<string, unknown>[] => Array.isArray(value) ? value.filter((entry): entry is Record<string, unknown> => Boolean(entry) && typeof entry === 'object') : [];
const recordValue = (value: unknown): Record<string, unknown> | null => Boolean(value) && typeof value === 'object' ? value as Record<string, unknown> : null;
...
export class AppStore {
  ...
  readonly widgets = signal<Array<{ id: string; name: string; url: string; description?: string; state?: string }>>([]);
  ...
  private readonly skillRunCursors = new Map<string, { sessionId: string; eventsURL: string; sequence: number }>();
  ...
  constructor(readonly config: AppConfig, readonly storage: Storage = localStorage) {
    ...
    this.selectedReasoningMode = signal(storage.getItem(this.keys.selectedReasoningMode) || 'standard');
    ...
    this.api = new APIClient(config, {
      ...
      onAuthRequired: () => { this.authRequired.value = true; this.modal.value = 'settings'; },
      onNetworkState: (state) => { this.networkState.value = state; this.connected.value = state === 'online'; },
      onVersionMismatch: () => { void this.hardRefresh(); },
    });
    this.endpoints = endpoints(this.api);
    this.activeSession = computed(() => this.sessions.value.find((session) => session.id === this.activeSessionId.value) || null);
    ...
    this.visibleMessages = computed(() => {
      const session = this.activeSession.value;
      if (!session) return [];
      const messages = mergeDurableProjection(session.messages, this.runs.value[session.id]?.messages || []);
      const clientIDs = new Set(messages.map((message) => message.clientMessageId).filter(Boolean));
      const pending = (this.pendingIntents.value[session.id] || []).filter((intent) => !clientIDs.has(intent.clientMessageId)).map((intent) => ({ id: intent.id, role: 'user' as const, content: intent.content, created: intent.created, clientMessageId: intent.clientMessageId, attachments: intent.attachments, pending: true }));
      return [...messages, ...pending];
    });
    this.streaming = computed(() => ['connecting', 'streaming', 'cancelling'].includes(this.activeProjection.value?.run.status || ''));
  }
  ...
  async bootstrap(): Promise<void> {
    ...
    try {
      ...
      const preferred = forceNew || restoreDraft ? '' : routed || this.storage.getItem(this.keys.activeSession) || '';
      const session = forceNew || restoreDraft ? null : this.sessions.value.find((entry) => entry.id === preferred || String(entry.number || '') === preferred) || this.sessions.value[0] || null;
      ...
      if (error instanceof APIError && [401, 403].includes(error.status)) { this.authRequired.value = true; this.modal.value = 'settings'; }
    }
  }
  ...
  private installLifecycle(): void {
    ...
    addEventListener('popstate', () => {
      ...
      const session = this.sessions.value.find((entry) => entry.id === slug || String(entry.number || '') === slug);
    ...
    addEventListener('offline', () => { this.networkState.value = 'offline'; this.connected.value = false; });
    addEventListener('term-llm:transport-fallback', () => void this.recover());
    addEventListener('pageshow', (event) => { if (this.startupDone.value || (event as PageTransitionEvent).persisted) void this.recover(); });
    addEventListener('focus', () => void this.refreshHubAgents());
    addEventListener('beforeunload', () => this.persistCurrentDraft());
    document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') { void this.recover(); void this.refreshHubAgents(); } });
    addEventListener('storage', (rawEvent) => {
      const event = rawEvent as StorageEvent;
      if (event.key === this.keys.pendingIntents || event.key?.startsWith(`${this.keys.pendingIntents}:`)) {
  ...
  private applyCapabilities(data: Record<string, unknown>): void {
    ...
    const worktrees = data.worktrees && typeof data.worktrees === 'object' ? data.worktrees as Record<string, unknown> : {};
  ...
  private applyWidgetStatus(value: unknown): void {
    const source = value && typeof value === 'object' && !Array.isArray(value) ? (value as Record<string, unknown>).widgets : value;
    this.widgets.value = array(source).map((entry) => {
      const mount = String(entry.mount || entry.id || '').replace(/^\/+|\/+$/g, '');
      return { id: String(entry.id || mount), name: String(entry.title || entry.name || mount || 'Widget'), description: String(entry.description || ''), state: String(entry.state || 'stopped'), url: mount ? `${this.config.prefix}/widgets/${encodeURIComponent(mount)}/` : String(entry.url || '') };
    }).filter((entry) => entry.url);
  }

  async loadWidgetStatus(): Promise<void> {
    try { this.applyWidgetStatus(await this.endpoints.widgetStatus()); } catch { /* Widgets are optional. */ }
  }
  ...
  private applySidebar(data: Record<string, unknown>): void {
    ...
    for (const group of groups) {
      ...
      const projectSource = group.project && typeof group.project === 'object' ? group.project as Record<string, unknown> : null;
      if (!projectSource || group.no_project) { ungrouped.push(...sessions.map((entry) => this.sessionFrom(entry))); continue; }
      const project: Project = {
        id: String(projectSource.id || ''), name: String(projectSource.name || projectSource.title || 'Project'), path: String(projectSource.canonical_dir || projectSource.path || ''),
        archived: Boolean(projectSource.archived_at || projectSource.archived), available: projectSource.available !== false, unavailableReason: String(projectSource.unavailable_reason || ''), git: projectSource.git === true,
        sessions: sessions.map((entry) => this.sessionFrom({ ...entry, project_id: projectSource.id, project_name: projectSource.name, project_unavailable: projectSource.available === false, project_unavailable_reason: projectSource.unavailable_reason })),
        sessionCount: Number(group.session_count) || sessions.length, next_cursor: String(group.next_cursor || ''), has_more: Boolean(group.next_cursor),
      };
      projects.push(project);
    }
    const incoming = [...ungrouped, ...projects.flatMap((project) => project.sessions || [])]; const existing = new Map(this.sessions.peek().map((session) => [session.id, session]));
    const merged = new Map(incoming.map((session) => [session.id, this.mergeSession(existing.get(session.id), session)]));
    for (const [id, session] of existing) if (!merged.has(id) && (this.runs.peek()[id] || id.startsWith('draft_'))) merged.set(id, session);
    this.sessions.value = [...merged.values()].sort((a, b) => Number(b.pinned) - Number(a.pinned) || b.lastMessageAt - a.lastMessageAt);
  ...
  private draftStorageID(): string { return this.activeSessionId.peek() || `draft:${this.activeProjectId.peek() || 'none'}`; }
  private persistCurrentDraft(): void {
    const id = this.draftStorageID();
    saveDraft(this.storage, this.keys.draftMessages, {
      sessionId: id, content: this.prompt.peek(), projectId: this.activeProjectId.peek(), updated: Date.now(),
      provider: this.selectedProvider.peek(), model: this.selectedModel.peek(), effort: this.selectedEffort.peek(), reasoningMode: this.selectedReasoningMode.peek(), agent: this.selectedAgent.peek(), worktreeDir: this.activeSession.peek()?.worktreeDir || this.selectedDraftWorktree.peek(),
      attachments: this.attachments.peek().filter((attachment) => !attachment.file),
    });
  }
  private restoreDraftFor(id: string): void {
    const draft = readDrafts(this.storage, this.keys.draftMessages).find((entry) => entry.sessionId === id);
    batch(() => { this.prompt.value = draft?.content || ''; this.attachments.value = draft?.attachments || []; this.selectedDraftWorktree.value = draft?.worktreeDir || ''; });
    if (!draft) return;
    if (draft.provider) this.setPreference('provider', draft.provider, false); if (draft.model) this.setPreference('model', draft.model, false); if (draft.effort) this.setPreference('effort', draft.effort, false); if (draft.reasoningMode) this.setPreference('reasoning', draft.reasoningMode, false); if (draft.agent) this.setPreference('agent', draft.agent, false);
  }
  private syncRuntimeFromSession(session: Session): void {
    ...
    if (session.activeReasoningMode) this.setPreference('reasoning', session.activeReasoningMode, false);
  }

  private async resolveAndSelectSession(id: string, replace: boolean): Promise<void> {
    try {
      const data = await this.endpoints.selectedSession(id); const source = recordValue(data.selected_session); if (!source) return this.newChat(replace);
      const session = this.sessionFrom(source); const existing = this.sessions.value.find((entry) => entry.id === session.id); if (!existing) this.sessions.value = [session, ...this.sessions.value];
  ...
  async loadSession(id: string, epoch = this.selectionEpoch): Promise<void> {
    ...
    try {
      ...
      const sideload = recordValue(selected.selected_transcript) || {}; const bodies = recordValue(sideload.bodies) || {};
      ...
      if (planSource && typeof planSource === 'object') {
        ...
        this.currentPlan.value = Array.isArray(plan) ? { explanation: String(raw.explanation || ''), plan: plan as CurrentPlan['plan'] } : null;
  ...
  async send(options: SendOptions = {}): Promise<void> {
    ...
    if (!session) {
      ...
      this.sessions.value = [session, ...this.sessions.value]; this.activeSessionId.value = session.id; this.draftActive.value = false;
    }
    const sessionId = session.id;
    let attachmentParts: Record<string, unknown>[];
    try { attachmentParts = await Promise.all(attachments.map((entry) => this.attachmentInput(entry))); }
    ...
    try {
      ...
      if (!session.lastResponseId && session.worktreeDir) requestBody.worktree_dir = session.worktreeDir;
      ...
      applyRuntimeToRequest(requestBody, { provider: session.activeProvider, model: session.activeModel, effort: session.activeEffort, reasoningMode: session.activeReasoningMode }, { provider: this.selectedProvider.value, model: this.selectedModel.value, effort: this.selectedEffort.value, reasoningMode: this.selectedReasoningMode.value }, selectedModel);
      const response = await this.endpoints.createResponse(requestBody, sessionId.startsWith('draft_') ? '' : sessionId, requestId, postAbort.signal);
      if (!response.ok || !response.body) {
        if (response.status === 409) {
          const body = await response.text();
          try { const parsed = JSON.parse(body) as { error?: { type?: string } }; if (parsed.error?.type === 'client_message_already_committed') { this.retireIntent(sessionId, clientMessageId); await this.loadSession(sessionId); return; } } catch { /* Normalized below. */ }
          throw new APIError(body || 'Response conflict', response.status, body);
        }
        throw new APIError((await response.text()) || `Response request returned ${response.status}`, response.status);
      }
  ...
  private rekeySession(oldID: string, id: string, source?: Record<string, unknown>): void {
    const current = this.sessions.value.find((entry) => entry.id === oldID); if (!current || !id) return;
    const incoming = source ? this.sessionFrom(source) : null; const updated: Session = { ...current, ...(incoming || {}), id, messages: current.messages };
    const duplicate = this.sessions.value.find((entry) => entry.id === id && entry.id !== oldID);
    this.sessions.value = this.sessions.value.filter((entry) => entry.id !== oldID && entry.id !== id);
    this.sessions.value = [this.mergeSession(duplicate, updated), ...this.sessions.value];
    const projection = this.runs.value[oldID]; const next = { ...this.runs.value }; delete next[oldID]; if (projection) next[id] = { ...projection, run: { ...projection.run, sessionId: id } }; this.runs.value = next;
    const intents = this.pendingIntents.value[oldID] || []; const registry = { ...this.pendingIntents.value }; delete registry[oldID]; if (intents.length) registry[id] = [...(registry[id] || []), ...intents]; this.pendingIntents.value = registry;
    removeSessionPendingIntents(this.storage, this.keys.pendingIntents, oldID); intents.forEach((intent) => persistPendingIntent(this.storage, this.keys.pendingIntents, id, intent));
    this.interjections.value = this.interjections.value.map((entry) => entry.sessionId === oldID ? { ...entry, sessionId: id } : entry);
    if (this.activeSessionId.peek() === oldID) { this.activeSessionId.value = id; this.storage.setItem(this.keys.activeSession, id); updateSessionRoute(this.config.prefix, updated, true); }
  }

  private async consumeResponseBody(body: ReadableStream<Uint8Array>, sessionId: string, signal?: AbortSignal): Promise<void> {
  ...
  async streamResponse(responseId: string, sessionId: string, after: number): Promise<void> {
    this.streamAborts.get(sessionId)?.abort(); const abort = new AbortController(); this.streamAborts.set(sessionId, abort);
    try {
      const response = await this.endpoints.responseEvents(responseId, after, abort.signal);
      if (!response.ok || !response.body) throw new Error(`Response stream returned ${response.status}`);
      await this.consumeResponseBody(response.body, sessionId, abort.signal);
      const projection = this.runs.value[sessionId];
      if (projection && ['connecting', 'streaming'].includes(projection.run.status) && !abort.signal.aborted) await this.resumeResponse(sessionId, responseId);
    } catch (error) {
      if (!abort.signal.aborted) {
        if (error instanceof ResponseProtocolError) { await this.resumeResponse(sessionId, responseId); return; }
        const projection = this.runs.value[sessionId];
        if (projection && ['connecting', 'streaming'].includes(projection.run.status)) {
          const reconnects = projection.run.reconnects + 1;
          this.runs.value = { ...this.runs.value, [sessionId]: { ...projection, run: { ...projection.run, reconnects } } };
          window.setTimeout(() => void this.streamResponse(responseId, sessionId, projection.run.lastSequence), Math.min(60_000, 1_000 * 1.5 ** Math.min(reconnects, 10)));
        } else this.failRun(sessionId, error);
      }
    } finally { if (this.streamAborts.get(sessionId) === abort) this.streamAborts.delete(sessionId); }
  }

  applyResponseEvent(sessionId: string, event: ResponseEvent): void {
    const current = this.runs.value[sessionId]; if (!current) return;
    if (event.type === 'response.stream_error') {
      const error = recordValue(event.error); if (String(error?.type || '') === 'stream_buffer_overflow') void this.resumeResponse(sessionId, current.run.responseId);
      return;
    }
    let next: ResponseProjection;
    try { next = reduceResponse(current, event); }
    catch (error) { if (error instanceof ResponseProtocolError) { void this.resumeResponse(sessionId, current.run.responseId); return; } throw error; }
    ...
    if (event.type === 'response.created' || event.type === 'response.completed') {
      ...
      if (Object.hasOwn(response, 'reasoning_effort')) runtimePatch.activeEffort = String(response.reasoning_effort || '');
      ...
      if (Object.hasOwn(event, 'reasoning_effort')) runtimePatch.activeEffort = String(event.reasoning_effort || '');
    } else if (event.type === 'response.model_swap.progress') {
      const prefix = event.stage === 'failed' ? 'previous_' : event.stage === 'complete' ? 'target_' : '';
      if (prefix) {
        ...
        if (Object.hasOwn(event, `${prefix}effort`)) runtimePatch.activeEffort = String(event[`${prefix}effort`] || '');
      }
    }
    if (event.type === 'response.completed' && next.usage) {
      const lastAssistant = [...next.messages].reverse().find((message) => message.role === 'assistant');
      if (lastAssistant) next = { ...next, messages: next.messages.map((message) => message === lastAssistant ? { ...message, usage: next.usage || undefined } : message) };
    ...
    if (['completed', 'cancelled', 'failed'].includes(next.run.status) && next.run.status !== current.run.status) {
      this.retireIntent(sessionId); this.sessions.value = this.sessions.value.map((session) => session.id === sessionId ? { ...session, activeResponseId: null, lastResponseId: next.run.responseId } : session);
      window.setTimeout(() => void this.refreshSessionMessages(sessionId, next.run.finalRev || 0), 0);
    }
  }

  private async refreshSessionMessages(sessionId: string, targetRev = 0): Promise<void> {
    try {
      const selected = await this.endpoints.selectedSession(sessionId); const source = recordValue(selected.selected_session); const sideload = recordValue(selected.selected_transcript); const bodies = recordValue(sideload?.bodies); if (!source || !bodies) return;
      const incoming = this.sessionFrom({ ...source, messages: listFrom(bodies, 'messages', 'items') }); if (targetRev && incoming.transcriptRev && incoming.transcriptRev < targetRev) return;
      this.sessions.value = this.sessions.value.map((session) => session.id === sessionId ? this.mergeSession(session, incoming) : session);
      this.retireCommittedIntents(sessionId, incoming.messages);
    } catch { /* Status polling will retry durable reconciliation. */ }
  }

  private async resumeResponse(sessionId: string, responseId: string): Promise<void> {
    try {
      const snapshot = await this.endpoints.response(responseId); const recovery = recordValue(snapshot.recovery) || {};
      const existing = this.runs.value[sessionId] || initialProjection({ responseId, sessionId, epoch: Number(snapshot.run_epoch) || 1, status: 'streaming', lastSequence: 0, startedRev: Number(snapshot.started_rev) || 0, reconnects: 0 });
      const rawMessages = listFrom(recovery, 'messages').length ? listFrom(recovery, 'messages') : listFrom(snapshot, 'messages');
      const projected: Message[] = rawMessages.map((raw, index) => {
        if (Array.isArray(raw.parts)) return convertServerMessages([raw], { rebaseAssetURL: (url) => rebaseHubAssetURL(this.config, url) })[0];
        return { ...raw, id: String(raw.id || `${responseId}:snapshot:${index}`), role: String(raw.role || 'assistant'), content: String(raw.content || raw.text || ''), created: Number(raw.created || raw.created_at) || Date.now(), responseId } as Message;
      }).filter(Boolean);
      ...
      const next: ResponseProjection = {
        ...
        run: { ...existing.run, responseId, epoch: Number(snapshot.run_epoch) || existing.run.epoch, status: status === 'failed' ? 'failed' : status === 'completed' ? 'completed' : status === 'cancelled' ? 'cancelled' : 'streaming', lastSequence: Number(snapshot.last_sequence_number ?? recovery.sequence_number) || existing.run.lastSequence, startedRev: Number(snapshot.started_rev) || existing.run.startedRev },
      };
      this.runs.value = { ...this.runs.value, [sessionId]: next };
      if (next.run.status === 'streaming') await this.streamResponse(responseId, sessionId, next.run.lastSequence);
      else await this.refreshSessionMessages(sessionId, Number(snapshot.final_rev) || 0);
    } catch (error) {
      const projection = this.runs.value[sessionId];
      if (projection && ['connecting', 'streaming'].includes(projection.run.status)) {
        window.setTimeout(() => void this.streamResponse(responseId, sessionId, projection.run.lastSequence), Math.min(60_000, retryBackoff(projection.run.reconnects)));
      } else this.failRun(sessionId, error);
    }
  }

  async cancel(): Promise<void> {
    const projection = this.activeProjection.value; if (!projection || projection.run.status === 'cancelling') return;
    this.runs.value = { ...this.runs.value, [projection.run.sessionId]: { ...projection, run: { ...projection.run, status: 'cancelling' } } };
    this.postAborts.get(projection.run.sessionId)?.abort(); this.streamAborts.get(projection.run.sessionId)?.abort();
    try { await this.endpoints.cancelResponse(projection.run.responseId); }
    catch (error) { this.toast(error, 'error'); }
  }

  async interject(content: string): Promise<void> {
    const session = this.activeSession.value; const value = content.trim(); if (!session || !value) return;
    const id = uuid(); const entry: PendingInterjection = { id, sessionId: session.id, content: value, state: 'sending' };
    this.interjections.value = [...this.interjections.value, entry];
    try {
      await this.endpoints.interrupt(session.id, { message: value, interjection_id: id, client_message_id: id, delivery: 'steer' }, id);
      this.interjections.value = this.interjections.value.map((candidate) => candidate.id === id ? { ...candidate, state: 'pending' } : candidate);
    } catch (error) {
      this.interjections.value = this.interjections.value.map((candidate) => candidate.id === id ? { ...candidate, state: 'failed' } : candidate); this.toast(error, 'error');
    }
  }
  async cancelInterjection(id: string): Promise<void> {
    const entry = this.interjections.value.find((candidate) => candidate.id === id); if (!entry) return;
    this.interjections.value = this.interjections.value.filter((candidate) => candidate.id !== id);
    try { await this.endpoints.deleteInterrupt(entry.sessionId, id); } catch (error) { this.toast(error, 'error'); }
  }

  private failRun(sessionId: string, error: unknown): void {
    const projection = this.runs.value[sessionId]; if (!projection) return;
    const message = error instanceof Error ? error.message : String(error);
    this.runs.value = { ...this.runs.value, [sessionId]: { ...projection, run: { ...projection.run, status: 'failed', error: message }, messages: [...projection.messages, { id: `error_${uuid()}`, role: 'error', content: message, created: Date.now() }] } };
    this.toast(message, 'error');
  }

  private retireCommittedIntents(sessionId: string, messages: Message[]): void {
    const committed = new Set(messages.map((message) => message.clientMessageId).filter((value): value is string => Boolean(value)));
    for (const intent of this.pendingIntents.peek()[sessionId] || []) if (committed.has(intent.clientMessageId)) this.retireIntent(sessionId, intent.clientMessageId);
  }

  private trackIntent(sessionId: string, intent: PendingIntentRegistry[string][number]): void {
    const registry = { ...this.pendingIntents.value, [sessionId]: [...(this.pendingIntents.value[sessionId] || []).filter((entry) => entry.clientMessageId !== intent.clientMessageId), intent] };
    this.pendingIntents.value = registry; persistPendingIntent(this.storage, this.keys.pendingIntents, sessionId, intent);
  }
  private retireIntent(sessionId: string, clientMessageId = ''): void {
    const registry = { ...this.pendingIntents.value };
    if (clientMessageId) registry[sessionId] = (registry[sessionId] || []).filter((entry) => entry.clientMessageId !== clientMessageId); else delete registry[sessionId];
    if (!registry[sessionId]?.length) delete registry[sessionId]; this.pendingIntents.value = registry;
    if (clientMessageId) this.storage.removeItem(`${this.keys.pendingIntents}:${encodeURIComponent(sessionId)}:${encodeURIComponent(clientMessageId)}`); else removeSessionPendingIntents(this.storage, this.keys.pendingIntents, sessionId);
  }

  async attachmentInput(attachment: Attachment): Promise<Record<string, unknown>> {
    let data = attachment.dataURL || attachment.url || '';
    if (!data && attachment.file) data = await new Promise<string>((resolve, reject) => {
      const reader = new FileReader(); reader.onload = () => resolve(String(reader.result || '')); reader.onerror = () => reject(reader.error || new Error(`Could not read ${attachment.name}`)); reader.readAsDataURL(attachment.file!);
    });
    if (!data) throw new Error(`Could not materialize ${attachment.name}`);
    if (attachment.type.startsWith('image/')) return { type: 'input_image', image_url: data, filename: attachment.name, ...(attachment.width && attachment.height ? { width: attachment.width, height: attachment.height } : {}) };
    return { type: 'input_file', file_data: data, filename: attachment.name };
  }

  addAttachments(files: FileList | File[]): void {
    for (const file of Array.from(files)) {
      const attachment: Attachment = { id: uuid(), name: file.name, type: file.type || 'application/octet-stream', size: file.size, file };
      if (file.type.startsWith('image/')) {
        const preview = URL.createObjectURL(file); attachment.previewURL = preview;
        const image = new Image(); image.onload = () => { attachment.width = image.naturalWidth; attachment.height = image.naturalHeight; this.attachments.value = [...this.attachments.value, attachment]; }; image.onerror = () => { this.attachments.value = [...this.attachments.value, attachment]; }; image.src = preview;
      } else this.attachments.value = [...this.attachments.value, attachment];
    }
  }
  removeAttachment(id: string | undefined): void {
    const attachment = this.attachments.value.find((entry) => entry.id === id); if (attachment?.previewURL?.startsWith('blob:')) URL.revokeObjectURL(attachment.previewURL);
    this.attachments.value = this.attachments.value.filter((entry) => entry.id !== id);
  }

  async search(query: string): Promise<void> {
    this.sidebarSearch.value = query; this.searchAbort?.abort(); clearTimeout(this.searchTimer);
    if (!query.trim()) { this.searchResults.value = null; this.searchLoading.value = false; this.searchError.value = ''; return; }
    this.searchLoading.value = true; this.searchResults.value = []; this.searchError.value = '';
    await new Promise<void>((resolve) => { this.searchTimer = window.setTimeout(resolve, 180); });
    if (this.sidebarSearch.peek() !== query) return;
    const abort = new AbortController(); this.searchAbort = abort;
    try {
      const data = await this.endpoints.searchSessions(query, this.showHidden.value, this.config.sidebarCategories, abort.signal);
      if (!abort.signal.aborted && this.sidebarSearch.peek() === query) this.searchResults.value = listFrom(data, 'sessions', 'items').map((entry) => this.sessionFrom(entry));
    } catch (error) { if (!abort.signal.aborted) { this.searchResults.value = null; this.searchError.value = error instanceof Error ? error.message : 'Could not search conversations'; } }
    finally { if (!abort.signal.aborted) this.searchLoading.value = false; }
  }
  ...
  async mutateSession(session: Session, patch: Record<string, unknown>): Promise<void> {
    ...
    this.sessions.value = this.sessions.value.map((entry) => entry.id === session.id ? { ...entry, ...patch } as Session : entry);
    await this.refreshSidebar();
  }
  async archiveSession(session: Session): Promise<void> { await this.mutateSession(session, { archived: !session.archived }); if (session.id === this.activeSessionId.value && !session.archived) this.newChat(); }
  async removeSession(session: Session): Promise<void> { await this.endpoints.deleteSession(session.id); this.sessions.value = this.sessions.value.filter((entry) => entry.id !== session.id); if (session.id === this.activeSessionId.value) this.newChat(); }
  async pinSession(session: Session): Promise<void> { await this.mutateSession(session, { pinned: !session.pinned }); }
  openRename(session: Session): void { this.renameTarget.value = session; this.modal.value = 'rename'; }
  openProjectPicker(session: Session): void { this.projectTarget.value = session; this.modal.value = 'project'; }
  async assignProject(projectId: string): Promise<void> { const session = this.projectTarget.value; if (!session) return; await this.endpoints.setProject(session.id, projectId); await this.refreshSidebar(); this.modal.value = ''; }
  async renameSession(title: string): Promise<void> { const session = this.renameTarget.value; if (!session) return; await this.mutateSession(session, { title: title.trim() || 'New chat', name: title.trim() }); this.modal.value = ''; }
  async improveTitle(): Promise<{ title: string; detail: string }> { const session = this.renameTarget.value; if (!session) return { title: '', detail: '' }; const data = await this.endpoints.refineTitle(session.id); return { title: String(data.title || ''), detail: String(data.detail || '') }; }
  ...
  saveSettings(token: string): void {
    this.token.value = token.trim(); if (this.token.value) this.storage.setItem(this.keys.token, this.token.value); else this.storage.removeItem(this.keys.token);
    syncTokenCookie(this.config.prefix, this.token.value); this.authRequired.value = false; this.modal.value = ''; void this.bootstrap();
  }
  ...
  async recoverSideQuestion(): Promise<void> {
    ...
    this.sideQuestion.value = {
      ...this.sideQuestion.value,
      running: Boolean(value.running), question: String(value.question || ''), response: String(value.response || ''),
      error: String(value.error || ''), history: Array.isArray(value.history) ? value.history as SideQuestionState['history'] : [],
    };
  }

  async askSideQuestion(question: string): Promise<void> {
    const session = this.activeSession.value; const value = question.trim(); if (!session || !value || this.sideQuestion.value.running) return;
    this.sideQuestionAbort?.abort(); const controller = new AbortController(); this.sideQuestionAbort = controller;
    this.sideQuestion.value = { ...this.sideQuestion.value, visible: true, running: true, question: value, response: '', error: '' };
    try {
      const response = await this.endpoints.startSideQuestion(session.id, value);
      if (!response.ok || !response.body) throw new Error((await response.text()) || `Side question failed (${response.status})`);
      let answer = '';
      for await (const frame of decodeSSE(response.body, controller.signal)) {
        let event: Record<string, unknown>; try { event = JSON.parse(frame.data) as Record<string, unknown>; } catch { continue; }
        if (event.type === 'text_delta') answer += String(event.text || ''); else if (event.type === 'attempt_discard') answer = ''; else if (event.type === 'done' && recordValue(event.result)) answer = String(recordValue(event.result)?.response || answer);
        this.sideQuestion.value = { ...this.sideQuestion.value, response: answer };
      }
      if (!controller.signal.aborted) { await this.recoverSideQuestion(); this.sideQuestion.value = { ...this.sideQuestion.value, visible: true, running: false }; }
    } catch (error) { if (!controller.signal.aborted) this.sideQuestion.value = { ...this.sideQuestion.value, running: false, error: error instanceof Error ? error.message : String(error) }; }
  }
  async closeSideQuestion(): Promise<void> {
    const session = this.activeSession.value; this.sideQuestionAbort?.abort();
    if (this.sideQuestion.value.running && session) await this.endpoints.cancelSideQuestion(session.id).catch(() => undefined);
    this.sideQuestion.value = { ...this.sideQuestion.value, visible: false, running: false };
  }

  async loadMCP(): Promise<void> { const session = this.activeSession.value; if (!session) return; this.mcp.value = { ...this.mcp.value, loading: true }; try { const data = await this.endpoints.getMCP(session.id); const servers = array(data.servers).map((server) => String(server.name || '')).filter(Boolean); this.mcp.value = { available: servers, enabled: (data.enabled || []) as string[], loading: false, error: '' }; } catch (error) { this.mcp.value = { ...this.mcp.value, loading: false, error: String(error) }; } }
  async toggleMCP(name: string): Promise<void> { const session = this.activeSession.value; if (!session) return; const enabled = this.mcp.value.enabled.includes(name) ? this.mcp.value.enabled.filter((entry) => entry !== name) : [...this.mcp.value.enabled, name]; await this.endpoints.setMCP(session.id, enabled); this.mcp.value = { ...this.mcp.value, enabled }; }
  async saveGoal(goal: Goal | { action: string }): Promise<void> { const session = this.activeSession.value; if (!session) return; const request = 'objective' in goal ? { action: 'set', objective: goal.objective, token_budget: goal.token_budget } : goal; await this.endpoints.goal(session.id, request); if ('objective' in goal) this.goal.value = goal; else if (goal.action === 'clear') this.goal.value = null; else if (this.goal.value) this.goal.value = { ...this.goal.value, status: goal.action === 'pause' ? 'paused' : 'active' }; this.modal.value = ''; }

  async shareLocation(): Promise<void> {
    if (!this.config.locationSharing || !navigator.geolocation) return this.toast('Location sharing is unavailable.', 'error');
    const position = await new Promise<GeolocationPosition>((resolve, reject) => navigator.geolocation.getCurrentPosition(resolve, reject, { enableHighAccuracy: false, timeout: 10_000 }));
  ...
  async loadDiff(): Promise<void> {
    const session = this.activeSession.value; if (!session) return; const owner = session.id; const scope = normalizeDiffScope(this.diff.value.scope);
  ...
  private worktreeProjectID(): string { return this.activeSession.value?.projectId || this.activeProjectId.value; }
  async loadWorktrees(): Promise<void> {
    const projectId = this.worktreeProjectID(); this.worktreeError.value = '';
    if (this.projectsEnabled.value && !projectId) { this.worktrees.value = []; this.worktreeError.value = 'Choose a project before selecting a worktree.'; return; }
    try { const data = this.projectsEnabled.value ? await this.endpoints.projectWorktrees(projectId) : await this.endpoints.legacyWorktrees(); this.worktrees.value = listFrom(data, 'worktrees', 'items'); }
    catch (error) { this.worktrees.value = []; this.worktreeError.value = worktreeError(error); }
  }
  async createWorktree(name: string): Promise<void> {
    const projectId = this.worktreeProjectID();
    try { if (this.projectsEnabled.value) await this.endpoints.createProjectWorktree(projectId, { name }); else await this.api.post('/v1/worktrees', { name }); await this.loadWorktrees(); }
    catch (error) { this.worktreeError.value = worktreeError(error); }
  }
  chooseDraftWorktree(dir: string): void {
    if (!this.draftActive.value) return; this.selectedDraftWorktree.value = dir;
    const id = this.draftStorageID(); const draft = readDrafts(this.storage, this.keys.draftMessages).find((entry) => entry.sessionId === id);
    saveDraft(this.storage, this.keys.draftMessages, { ...(draft || { sessionId: id, content: this.prompt.value, updated: Date.now() }), worktreeDir: dir, projectId: this.activeProjectId.value }); this.modal.value = '';
  }
  async worktreeDiff(dir: string): Promise<string> { const data = await this.endpoints.worktreeDiff(this.worktreeProjectID(), dir); return String(data.diff || data.patch || ''); }
  async mergeWorktree(dir: string): Promise<void> { await this.endpoints.mergeWorktree(this.worktreeProjectID(), dir); await this.loadWorktrees(); this.toast('Worktree merged.', 'success'); }
  ...
  private updateSkillRunMessage(sessionId: string, runId: string, patch: Record<string, unknown>): void {
    this.sessions.value = this.sessions.value.map((session) => {
      ...
      const messages = session.messages.map((message) => {
        ...
        const output = String(patch.output ?? message.output ?? ''); const error = String(patch.error ?? message.error ?? ''); const progress = String(patch.progress ?? message.progress ?? '');
        return { ...message, ...patch, status, output, error, progress, content: error || output || progress };
      });
      return { ...session, messages };
    });
  }
  private applySkillRunEvent(runId: string, envelope: Record<string, unknown>): void {
    const cursor = this.skillRunCursors.get(runId); if (!cursor) return;
    const sequence = Number(envelope.sequence || envelope.sequence_number) || 0; if (sequence && sequence <= cursor.sequence) return; if (sequence) cursor.sequence = sequence;
    const type = String(envelope.type || ''); const data = recordValue(envelope.data) || envelope; const patch: Record<string, unknown> = {};
    if (type === 'skill_run.created') Object.assign(patch, { status: 'running', skill: String(data.skill || ''), agent: String(data.agent || ''), childSessionId: String(data.child_session_id || '') });
    else if (type === 'skill_run.progress') patch.progress = String(data.message || data.progress || data.stage || 'Working…');
    else if (type === 'skill_run.completed') Object.assign(patch, { status: String(data.status || 'completed'), output: String(data.output || ''), error: String(data.error || ''), progress: '', childSessionId: String(data.child_session_id || '') });
    else return;
    this.updateSkillRunMessage(cursor.sessionId, runId, patch);
    if (this.skillRunTerminal(patch.status)) { this.skillRunAborts.get(runId)?.abort(); this.skillRunCursors.delete(runId); void this.refreshSessionMessages(cursor.sessionId); }
  }
  private async reconcileSkillRun(runId: string): Promise<void> {
    const cursor = this.skillRunCursors.get(runId); if (!cursor) return;
    const snapshot = await this.endpoints.skillRun(cursor.sessionId, runId); const events = Array.isArray(snapshot.events) ? snapshot.events : [];
    events.forEach((event) => { if (event && typeof event === 'object') this.applySkillRunEvent(runId, event as Record<string, unknown>); });
    this.updateSkillRunMessage(cursor.sessionId, runId, { status: String(snapshot.status || 'running'), output: String(snapshot.output || ''), error: String(snapshot.error || ''), childSessionId: String(snapshot.child_session_id || '') });
    if (this.skillRunTerminal(snapshot.status)) { this.skillRunCursors.delete(runId); await this.refreshSessionMessages(cursor.sessionId); }
  }
  private async followSkillRun(runId: string): Promise<void> {
    const cursor = this.skillRunCursors.get(runId); if (!cursor || this.skillRunAborts.has(runId)) return;
    ...
    try {
      ...
      for await (const frame of decodeSSE(response.body, controller.signal)) {
        ...
        this.applySkillRunEvent(runId, envelope); if (!this.skillRunCursors.has(runId)) return;
      }
    } catch (error) { if (!controller.signal.aborted) this.updateSkillRunMessage(cursor.sessionId, runId, { progress: navigator.onLine === false ? 'Offline — run is safe; reconnecting when online' : 'Reconnecting…', streamError: String(error) }); }
    finally { if (this.skillRunAborts.get(runId) === controller) this.skillRunAborts.delete(runId); }
    if (!this.skillRunCursors.has(runId)) return;
    await this.reconcileSkillRun(runId).catch(() => undefined);
    if (this.skillRunCursors.has(runId)) window.setTimeout(() => void this.followSkillRun(runId), 1_000);
  }
  async invokeSkill(name: string, args: string): Promise<void> {
    const session = this.activeSession.value; if (!session) return; const id = uuid(); const invocation = `/${name}${args.trim() ? ` ${args.trim()}` : ''}`;
    const optimistic: Message = { id: `pending_${id}`, role: 'user', content: invocation, clientMessageId: id, created: Date.now() };
    this.sessions.value = this.sessions.value.map((entry) => entry.id === session.id ? { ...entry, messages: [...entry.messages, optimistic], lastMessageAt: Date.now() } : entry); this.trackIntent(session.id, { id: optimistic.id, clientMessageId: id, content: invocation, created: optimistic.created });
    try {
      const data = await this.endpoints.invokeSkill(session.id, { name, arguments: args, client_message_id: id }, id);
      if (String(data.execution) === 'isolated' && data.run_id) {
        const runId = String(data.run_id); const eventsURL = String(data.events_url || `/v1/sessions/${encodeURIComponent(session.id)}/skill-runs/${encodeURIComponent(runId)}/events`);
        const message: Message = { id: `skill-run-${runId}`, role: 'skill-run', content: '', created: Date.now(), runId, skill: name, status: String(data.status || 'running'), childSessionId: String(data.child_session_id || ''), eventsURL };
        this.sessions.value = this.sessions.value.map((entry) => entry.id === session.id ? { ...entry, messages: [...entry.messages, message] } : entry);
        this.skillRunCursors.set(runId, { sessionId: session.id, eventsURL, sequence: 0 }); void this.followSkillRun(runId);
      } else {
        const responseId = String(data.response_id || ''); const epoch = Number(data.run_epoch) || 0;
        if (!responseId || !epoch) throw new Error('Skill invocation did not return a response ID and run epoch.');
        const run: ActiveRun = { responseId, sessionId: session.id, epoch, status: 'streaming', lastSequence: 0, startedRev: Number(data.started_rev) || session.transcriptRev || 0, reconnects: 0, requestId: id };
        this.runs.value = { ...this.runs.value, [session.id]: initialProjection(run) }; this.sessions.value = this.sessions.value.map((entry) => entry.id === session.id ? { ...entry, activeResponseId: responseId } : entry); void this.streamResponse(responseId, session.id, 0);
      }
      this.modal.value = ''; this.toast(`Started ${name}.`, 'success');
    } catch (error) { this.retireIntent(session.id, id); if (!this.prompt.value) this.prompt.value = invocation; this.toast(error, 'error'); }
  }
  async cancelSkill(runId: string): Promise<void> { const session = this.activeSession.value; if (!session) return; this.updateSkillRunMessage(session.id, runId, { status: 'cancelling', progress: 'Cancelling…' }); await this.endpoints.cancelSkillRun(session.id, runId); if (this.skillRunCursors.has(runId)) await this.reconcileSkillRun(runId).catch(() => this.refreshSessionMessages(session.id)); else await this.refreshSessionMessages(session.id); }

  async mutateTranscript(operation: 'undo' | 'redo'): Promise<void> {
    const session = this.activeSession.value; if (!session || this.streaming.value) return this.toast(`Cannot ${operation} while work is active.`, 'error');
    const durable = [...session.messages].reverse().find((message) => message.durableRowId); const owner = session.id;
    try {
      const result = await this.endpoints.mutateTranscript(owner, operation, { expected_rev: session.transcriptRev || 0, expected_head_id: durable?.durableRowId || 0 });
      await this.refreshSessionMessages(owner, Number(result.rev) || 0);
      if (this.activeSessionId.peek() === owner) this.prompt.value = operation === 'undo' ? String(result.user_text || '') : '';
      this.toast(operation === 'undo' ? (result.attachments_omitted ? 'Removed the latest turn. Attachments were not restored.' : 'Removed the latest turn. Your prompt is back in the composer.') : 'Restored the undone turn.', result.attachments_omitted ? 'error' : 'success');
      if (this.diff.value.open) void this.loadDiff();
    } catch (error) { if (error instanceof APIError && error.status === 409) await this.refreshSessionMessages(owner); this.toast(error, 'error'); }
  }

  async loadMoreProject(projectId: string): Promise<void> {
    const project = this.projects.value.find((entry) => entry.id === projectId); if (!project?.next_cursor) return;
    const data = await this.endpoints.projectSessions(projectId, project.next_cursor, this.showHidden.value); const incoming = listFrom(data, 'sessions', 'items').map((entry) => this.sessionFrom({ ...entry, project_id: project.id, project_name: project.name }));
    const existing = new Map(this.sessions.value.map((entry) => [entry.id, entry])); incoming.forEach((entry) => existing.set(entry.id, this.mergeSession(existing.get(entry.id), entry))); this.sessions.value = [...existing.values()];
    this.projects.value = this.projects.value.map((entry) => entry.id === projectId ? { ...entry, sessions: [...(entry.sessions || []), ...incoming.filter((candidate) => !(entry.sessions || []).some((session) => session.id === candidate.id))], next_cursor: String(data.next_cursor || ''), has_more: Boolean(data.next_cursor) } : entry);
  }
  async mutateProject(project: Project, patch: Record<string, unknown>): Promise<void> { await this.endpoints.patchProject(project.id, patch); await this.refreshSidebar(); }
  async startProjectChat(projectId: string): Promise<void> { this.newChat(false, projectId); this.sidebarOpen.value = false; }

  async refreshHubAgents(force = false): Promise<void> {
    if (this.hubAgentFetch) return this.hubAgentFetch; if (document.visibilityState === 'hidden') return;
    const raw = this.config.hub?.url || ''; if (!raw) { this.hubAgents.value = []; return; }
    let hub: URL; try { hub = new URL(raw, location.href); } catch { return; }
    if (hub.origin !== location.origin) { this.hubAgents.value = []; return; }
    if (!force && Date.now() - this.hubAgentLastFetch < 60_000) return;
    this.hubAgentLastFetch = Date.now(); const controller = new AbortController();
    const request = (async () => {
      try {
        const path = `${hub.pathname.replace(/\/+$/, '')}/api/nodes`; const data = await this.endpoints.hubNodes(new URL(path, location.origin).href, controller.signal);
        const safePath = (value: unknown): string => typeof value === 'string' && /^\/(?![\\/])/.test(value) ? value : '';
        const target = (node: Record<string, unknown>): string => {
          const sessions = recordValue(node.sessions) || {}; const active = array(sessions.active); const recent = array(sessions.recent);
          return safePath(sessions.resume_path) || safePath(active[0]?.resume_path) || safePath(recent[0]?.resume_path) || safePath(node.new_session_path) || (safePath(node.proxy_path) ? `${safePath(node.proxy_path)}?new=1` : '');
        };
        const previous = new Map(this.hubAgents.peek().map((entry) => [entry.id, entry]));
        this.hubAgents.value = array(data.nodes).filter((node) => recordValue(node.status)?.reachable === true).map((node) => {
          const id = String(node.id || ''); const sessions = recordValue(node.sessions) || {}; const active = Number(sessions.active_count) > 0 || array(sessions.active).length > 0; const old = previous.get(id);
          return { id, name: String(node.name || id), target: target(node), active, attention: id !== this.config.hub?.nodeId && Boolean(old?.attention || (old?.active && !active)) };
        }).filter((entry) => entry.name && entry.target).sort((left, right) => left.name.toLowerCase().localeCompare(right.name.toLowerCase()) || left.id.localeCompare(right.id));
      } catch (error) { if (error instanceof APIError && error.status >= 400 && error.status < 500) this.hubAgents.value = []; }
    })(); this.hubAgentFetch = request.finally(() => { this.hubAgentFetch = null; }); return this.hubAgentFetch;
  }
  clearHubAttention(id: string): void { this.hubAgents.value = this.hubAgents.value.map((entry) => entry.id === id ? { ...entry, attention: false } : entry); }

  private startStatusPoll(): void {
    clearTimeout(this.statusTimer);
    const poll = async () => {
      if (document.visibilityState === 'visible') await this.refreshStatus().catch(() => undefined);
      const anyActive = Object.values(this.runs.peek()).some((projection) => ['connecting', 'streaming', 'cancelling'].includes(projection.run.status));
      this.statusTimer = window.setTimeout(poll, anyActive ? 2_000 : 30_000);
    };
    this.statusTimer = window.setTimeout(poll, 30_000);
  }
  private async refreshStatus(): Promise<void> {
    const data = await this.endpoints.sessionStatus(this.activeSessionId.peek()); const statuses = listFrom(data, 'sessions', 'items');
    const byID = new Map(statuses.map((entry) => [String(entry.id || entry.session_id), entry]));
    this.sessions.value = this.sessions.value.map((session) => {
      const status = byID.get(session.id); if (!status) return session;
      const activeResponseId = String(status.active_response_id || '') || null; const transcriptRev = Number(status.transcript_rev) || session.transcriptRev || 0;
      if (activeResponseId && activeResponseId !== session.activeResponseId) window.setTimeout(() => void this.resumeResponse(session.id, activeResponseId), 0);
      if (!activeResponseId && session.activeResponseId && transcriptRev >= (this.runs.peek()[session.id]?.run.finalRev || 0)) window.setTimeout(() => void this.refreshSessionMessages(session.id, transcriptRev), 0);
      return { ...session, activeResponseId, lastResponseId: String(status.last_response_id || '') || session.lastResponseId, transcriptRev, messageCount: Number(status.message_count) || session.messageCount, lastMessageAt: Number(status.last_message_at) ? Number(status.last_message_at) * (Number(status.last_message_at) < 10_000_000_000 ? 1000 : 1) : session.lastMessageAt };
    });
  }

  toast(value: unknown, kind: Toast['kind'] = 'info'): void { const message = value instanceof Error ? value.message : String(value); const toast = { id: uuid(), message, kind }; this.toasts.value = [...this.toasts.value, toast]; window.setTimeout(() => { this.toasts.value = this.toasts.value.filter((entry) => entry.id !== toast.id); }, 4000); }
  async hardRefresh(): Promise<void> { await hardRefreshAssets(this.config); }
}
