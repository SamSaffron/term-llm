...
export function Lightbox() {
  ...
  return <div class={`lightbox active ${maximized ? 'lightbox-maximized' : ''}`} role="dialog" aria-modal="true" aria-label="Media preview">
    ...
    <div class="lightbox-toolbar"><button class="lightbox-btn" type="button" aria-label="Copy URL" onClick={() => void copyText(media.src)}><Icon name="copy" /></button><button class="lightbox-btn" type="button" aria-label={maximized ? 'Collapse' : 'Expand'} onClick={() => setMaximized(!maximized)}><Icon name={maximized ? 'restore' : 'expand'} /></button><button class="lightbox-btn" type="button" aria-label="Close" onClick={() => { store.lightbox.value = null; setMaximized(false); }}><Icon name="close" /></button></div>
  </div>;
}
