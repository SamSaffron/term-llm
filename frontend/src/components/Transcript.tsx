...
function MessageMeta({ message }: { message: Message }) {
  return <div class="message-meta"><span class="message-role">{message.role === 'user' ? 'You' : message.role === 'assistant' ? 'Assistant' : 'Notice'}</span><span class="message-meta-separator" aria-hidden="true">·</span><time title={new Date(message.created).toLocaleString()}>{relativeTime(message.created)}</time></div>;
}
...
function MessageRow({ message, streaming, turnText }: { message: Message; streaming: boolean; turnText: string }) {
  ...
  if (message.role === 'tool-group') return <div class="tool-group" data-message-id={message.id}>{message.tools?.map((tool) => <Tool key={tool.id} tool={tool} />)}</div>;
  ...
  return <article class={`message ${message.role} ${message.askUser ? 'ask-user-answer' : ''}`} data-message-id={message.id} data-created={message.created} data-durable-id={message.durableRowId} dir="auto">
    ...
    {message.usage && <div class="usage-line">{Number(message.usage.total_tokens || 0).toLocaleString()} tokens</div>}
    {message.role === 'assistant' && message.content && <div class="turn-action-panel"><button class="turn-action-btn message-copy" type="button" aria-label="Copy response" onClick={() => void copyText(turnText || message.content)}><Icon name="copy" /></button>{message.durableRowId && <button class="text-action" type="button" onClick={() => void store.branchFrom(String(message.durableRowId), 'clean')}>Branch from here</button>}</div>}
  </article>;
}
