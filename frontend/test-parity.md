...
| `markdown-setup.js`, `markdown-streaming.js`, `decoration.js`, `guardian-render.js` | marked configuration, DOMPurify policy, stable streaming boundary, adaptive render throttling and bounded over-budget fallback, strict math delimiters, lazy highlight/KaTeX, media and guardian rendering | `src/domain/{markdown,markdown-streaming}.ts`, Preact `Markdown`/`Transcript`/`Lightbox`; rendering and component tests |
...
| `slash-commands.js`, `app-composer.js` | built-in and live skill commands, streaming-safe filtering, debounced project/worktree mention completion alongside agent mentions, textarea keys, voice/transcription, send/interject controls | `domain/completions.ts`, Preact `Composer`, `platform/voice.ts`; domain/component/browser tests |
...
| `app-skills.js` | per-session skill discovery, slash invocation, isolated/main execution and run status | `AppStore`, `domain/completions.ts`, typed endpoint ownership and `skill-run` transcript rows; store/domain and retained endpoint tests |
...
| `app-webrtc.js` | optional signaling/data-channel transport, HTTPS fallback, abort/cancel, chunk-delivery degradation, watchdog and renegotiation | ESM `platform/webrtc.ts`, loaded only after deferred config parsing; frame protocol preserved and covered by `platform/webrtc.test.ts`, Go chunk/MIME tests, and environment smoke |
...
| `mention_completions_test.js`, `slash_commands_test.js` | Ported to `domain/rendering.test.ts` and Composer component tests, including debounce/abort context, range replacement, live skills, streaming-safe commands, and Escape preservation. |
...
| `app_webrtc_test.js` | Transport remains an ESM platform bridge; protocol/fallback regressions are ported to `platform/webrtc.test.ts`, with Go optional chunk/MIME tests and environment smoke. |
| `sw_test.js` | Cache/version/chunk URL semantics are covered by Go `RenderServiceWorker` tests; payload tooling derives the exact cache allowlist and records zero install-time requests. |
