...
| --- | --- | --- | --- | --- |
| M0 | Dependency-free analyzer, counting tests, initial/current baselines, CI ratchet, contract map | repository: 849/10,931 files/functions → 906/11,088; median 3 unchanged; >20 443 → 458; >50 58 → 55; >100 12 → 9. The >20 increase is from narrow extracted functions' base scores and is tracked by the family rule. | Full/race commands recorded below | Accepted after final gates |
...
| M6 | Request-history builder, synchronized event accumulator with immutable snapshots, text/message-window presentation and media delivery | `streamReplyContinuation` 245/1,274 → 157/815; extracted presentation/accumulator methods below 20 | Lifecycle intentionally retains chat lock, watchdog, stream and persistence ownership | Accepted after serve race gate |
...
The contract-anchor suites listed above predate base revision `4363767c` and
were used to characterize the original entry points. The two missing narrow
cases—repeat-invocation flag restoration and accumulator snapshot isolation—are
kept in the separate regression-coverage commit. Mechanical line-multiset
review confirmed that the store, chat-command, and serve-test family splits
remove no source statements; their residual differences are package/import
boilerplate.
