...
func TestSearchCredentialsExaMCPEnvFallbackOnlyForOfficialURL(t *testing.T) {
	...
	if got, err := official.ExaMCPKey(official.ExaMCP.URL).Resolve(); err != nil || got != "env-key" {
	...
	if got, err := custom.ExaMCPKey(custom.ExaMCP.URL).Resolve(); err != nil || got != "" {
	...
	if got, err := explicit.ExaMCPKey(explicit.ExaMCP.URL).Resolve(); err != nil || got != "env-key" {
