...
var keySpecs = []KeySpec{
	...
	optional("gateway.token", sensitive()),
	optional("gateway.token_file"),
	def("gateway.token_env", DefaultGatewayTokenEnv),
	def("gateway.local_providers", []string{}),
	def("gateway.search", true),
	def("gateway.fetch", true),
	def("gateway.required", false),
	def("gateway.catalog_ttl", DefaultGatewayCatalogTTL),
	def("gateway.connect_timeout", DefaultGatewayConnectTimeout),
	def("gateway.response_timeout", DefaultGatewayResponseTimeout),
	def("gateway.idle_timeout", DefaultGatewayIdleTimeout),
	def("gateway.tool_timeout", DefaultGatewayToolTimeout),
...
var providerFieldSpecs = []ProviderFieldSpec{
	...
	{Path: "allow_unlisted_models", Placeholder: false},
	{Path: "base_url"},
	{Path: "url"},
