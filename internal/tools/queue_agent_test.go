...
func TestQueueAgentRecoversRunAfterLostTriggerResponse(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "run visibility is delayed", emptyLookups: 2},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			if got, want := atomic.LoadInt32(&runLookups), test.emptyLookups+1; got != want {
				t.Fatalf("run lookups = %d, want %d", got, want)
			}
		})
	}
}
