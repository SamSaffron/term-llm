...
func TestShellToolBlankAffectedPathsUseFallbackTracking(t *testing.T) {
	dir := t.TempDir()
	...
	ctx := trackingContext()
	snap := preShellSnapshot(ctx, recorder, dir, []string{"", "  "})

	if calls := recorder.sessionPathCallCount(); calls != 1 {
		t.Fatalf("SessionPaths calls = %d, want 1 for blank affected_paths", calls)
	}
	if _, ok := snap.files[tracked]; !ok {
		t.Fatal("blank affected_paths disabled fallback tracking")
	}
}
