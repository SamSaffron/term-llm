...
func TestSpawnAgentToolPermittedNamesDoesNotMutateCatalogSlice(t *testing.T) {
	runner := newMockCatalogRunner("zeta", "alpha")
	runner.borrowNames = true
	...
	if err != nil || len(names) != 2 || names[0] != "alpha" || names[1] != "zeta" {
		t.Fatalf("PermittedAgentNames() = %#v, err=%v", names, err)
	}
	if runner.names[0] != "zeta" || runner.names[1] != "alpha" {
		t.Fatalf("catalog-owned names mutated to %#v", runner.names)
	}
}
