...
func captureRipgrepOutputWithConsumer(stdout io.Reader, limits ripgrepCaptureLimits, consumeLine ripgrepLineConsumer) ([]byte, bool, error) {
	...
	for {
		...
		if len(fragment) > 0 {
			if out.Len()+pendingLine.Len()+len(fragment) > limits.maxBufferedBytes {
				return out.Bytes(), true, nil
			}
			pendingLine.Write(fragment)
		}
		...
		case errors.Is(readErr, io.EOF):
			// Preserve a final unterminated line on normal EOF. Truncation paths above
...
func (t *GrepTool) Execute(ctx context.Context, args json.RawMessage) (llm.ToolOutput, error) {
	...
	if t.approval != nil {
		...
		if outcome == Cancel {
			return pathApprovalErrorOutput(warning, NewToolErrorf(ErrPermissionDenied, "access denied: %s", searchPath)), nil
		}
	}
