...
test('enrolls and authenticates with a virtual passkey', async ({ page, context }) => {
  ...
  const { authenticatorId: primaryAuthenticatorId } = await cdp.send('WebAuthn.addVirtualAuthenticator', { options: {
  ...
  await page.getByLabel('One-time setup code').fill(process.env.TERM_LLM_HUB_SMOKE_TOKEN);
  ...
  const sessionCookie = (await context.cookies()).find(cookie => cookie.name === 'term_llm_hub_session');
  expect(sessionCookie).toMatchObject({ httpOnly: true, secure: true, sameSite: 'Strict', path: '/hub/' });
  ...
  page.once('dialog', dialog => dialog.accept('Virtual second key'));
  ...
  page.once('dialog', dialog => dialog.accept('Renamed virtual key'));
  ...
  await expect(page.locator('.delegation-card').filter({ hasText: 'Virtual platform passkey' }).getByRole('button', { name: 'Remove' })).toBeDisabled();
  ...
  // Losing the cookie while the dashboard is open must redirect instead of
  // rendering an empty credential/session state.
  await page.getByRole('button', { name: 'Security' }).click();
  ...
  await expect(page).toHaveURL(/\/hub\/auth\/login$/);
  const { credentials } = await cdp.send('WebAuthn.getCredentials', { authenticatorId: primaryAuthenticatorId });
  expect(credentials).toHaveLength(1);
  fs.writeFileSync(process.env.TERM_LLM_HUB_CREDENTIAL_FILE, JSON.stringify(credentials[0]));
});
