...
async function testConversationBranchTreeOpensAndSwitchesPaths() {
  ...
  app.apiFetch = async (url) => {
    ...
    if (String(url).includes(`/sessions/${root.id}/tree`)) {
      return new Response(JSON.stringify({
  ...
  if (app.elements.branchTreeBtn.hidden
      ...
      || sections[1].children.length !== 2) {
    fail(name, 'tree did not render existing paths and branch points');
    return;
  }
...
async function testMarkedPathNoteConvertsToVisibleArtifact() {
  const name = 'marked path note converts to a visible non-authoritative artifact';
  const { app } = await createSessionsHarness();
  const converted = app.convertServerMessages([{
    id: 91,
    sequence: 4,
    role: 'path-note',
    created_at: 1710000000000,
    parts: [{ type: 'path_note', text: '- Useful parser finding.', path_note: { source_session_id: 'source', anchor_message_id: 7, modified_files: ['parser.go'] } }]
  }]);
  if (converted.length !== 1 || converted[0].role !== 'path-note'
      || converted[0].content !== '- Useful parser finding.'
      || converted[0].provenance?.source_session_id !== 'source') {
    fail(name, 'path note conversion mismatch', JSON.stringify(converted));
    return;
  }
  pass(name);
}
...
const runAppSessionsTest = async (testCase) => {
  const index = appSessionsTestIndex++;
  if (appSessionsShard === null || index % 2 === appSessionsShard) await testCase();
};
...
(async () => {
  ...
  await runAppSessionsTest(testOrdinaryStateSyncConvergesMissedPlanUpdate);
  await runAppSessionsTest(testSessionStatePlanRequestsIgnoreOlderOverlaps);
  await runAppSessionsTest(testSessionStatePlanRequestsKeepNewerAuthoritativeClear);
  await runAppSessionsTest(testSessionSwitchClearsPlanBeforeRejectingOldResponse);
  await runAppSessionsTest(testSessionSwitchClearsProviderRetryOwner);
  await runAppSessionsTest(testStoppedRunReconciliationClearsProviderRetryOwner);
  await runAppSessionsTest(testSwitchingSessionsStagesCurrentComposerBeforeRestore);
  ...
  await runAppSessionsTest(testNumericDeepLinkResolvesRealSessionId);
  await runAppSessionsTest(testIdleStartupSchedulesNormalPollWithoutImmediateRequest);
  await runAppSessionsTest(testStartupTranscriptSideloadRendersBoundedFirstPaintWithoutTranscriptRequests);
  await runAppSessionsTest(testSelectedTranscriptSideloadReconcilesProjectedTailAcrossSwitchBack);
  await runAppSessionsTest(testMalformedStartupTranscriptSideloadFallsBack);
  await runAppSessionsTest(testStartupTranscriptSideloadRevisionRaceFetchesOnlyNewerRevision);
  ...
  await runAppSessionsTest(testSidebarSwitchSkipsLoadedAndUnresolvedSessionRequests);
  await runAppSessionsTest(testStartupSplashWaitsForDeepLinkedTranscriptRender);
  await runAppSessionsTest(testStartupSideloadsWidgetsBeforeFirstPaint);
  await runAppSessionsTest(testStartupSideloadsEmptyWidgetList);
  await runAppSessionsTest(testExplicitWidgetRefreshStillUsesStatusEndpoint);
  await runAppSessionsTest(testStartupHydrationTimeoutDoesNotBlockIndefinitely);
  ...
  await runAppSessionsTest(testConvertServerMessagesCompactionSummariesBecomeMarkers);
  await runAppSessionsTest(testConvertServerMessagesSuppressesCompactionRetainedRawTail);
  await runAppSessionsTest(testCompactionDuplicateTailRangeIsLinear);
  await runAppSessionsTest(testConvertServerMessagesSuppressesAuthoritativeCompactionTailFlag);
  ...
  await runAppSessionsTest(testConvertServerMessagesSuppressesNonBubbleAssistantRows);
  await runAppSessionsTest(testSessionPruningDestroysConversationControllers);
  await runAppSessionsTest(testPendingIntentStorageMergesConcurrentWriters);
  await runAppSessionsTest(testAskUserDurableAcknowledgementClearsIntentStorage);
  await runAppSessionsTest(testOptimisticTranscriptStorageIsPerSession);
  await runAppSessionsTest(testLegacyUnversionedStorageMigratesOnlyIntent);
  await runAppSessionsTest(testReloadMigratesLegacyProjectionShadowsAtEqualRevision);
  await runAppSessionsTest(testReloadRetiresStalePersistedAssistantIdentityAfterTerminalAdvance);
  await runAppSessionsTest(testReloadScopesStalePersistedRetirementToCurrentActiveResponse);
  await runAppSessionsTest(testTranscriptSyncCommitsOneRenderedProjection);
  await runAppSessionsTest(testBottomPinnedTranscriptSyncKeepsFollowingTail);
  await runAppSessionsTest(testSatisfiedInflightRevisionDoesNotRefetchTranscript);
  await runAppSessionsTest(testInflightSyncAbsorbsQueuedZeroRevisionActivation);
  await runAppSessionsTest(testTranscriptProjectionAnnotatesCompactionBoundaryOnceAcrossSegments);
  await runAppSessionsTest(testGroupedToolRowsPreserveDurableRangesAndLaterAnchors);
  await runAppSessionsTest(testLargeToolTurnLoadsAsOneSegmentWithFarEndGrouping);
  await runAppSessionsTest(testTerminalTranscriptSyncQueuesBehindInflightRequest);
  await runAppSessionsTest(testSatisfiedTerminalSyncDoesNotRefetchTranscript);
  await runAppSessionsTest(testUntargetedForceQueuesBehindInflightRequest);
  await runAppSessionsTest(testForceScrollQueuesBehindSatisfiedInflightRequest);
  await runAppSessionsTest(testSwitchToSessionSyncsWithoutTokenAndResumes);
  await runAppSessionsTest(testSwitchToSessionAttachesChangedActiveResponseFromStartedRevision);
  await runAppSessionsTest(testActiveStatusSyncsInRunTranscriptRevisionsWithoutDuplicatingActiveOutput);
  await runAppSessionsTest(testHugeTranscriptGapTraversalStaysBoundedAndAnchored);
  await runAppSessionsTest(testIdleSessionSyncRescuesPendingInterruptCommit);
  await runAppSessionsTest(testIdleStateRetiresLostTerminalProjectionFromCurrentDurableTranscript);
  await runAppSessionsTest(testStaleIdleStateCannotRetireNewerActiveProjection);
  await runAppSessionsTest(testIdleStateRetiresOnlySampledTransportOwnership);
  await runAppSessionsTest(testIdleSidebarStatusPreservesUnacknowledgedPost);
  await runAppSessionsTest(testSessionProgressStatePrefersLocalAndServerSignals);
  await runAppSessionsTest(testResumeAndDrainFiringViaSync);
  ...
  await runAppSessionsTest(testVisibilityResumeRestoresPreBackgroundTailOwnership);
  await runAppSessionsTest(testPageshowWaitsForInitialConnectionBeforeStatusPoll);
  await runAppSessionsTest(testSidebarStatusPollRecoversIdempotentlyAfterPageShow);
  await runAppSessionsTest(testHiddenInFlightSidebarPollCannotRescheduleOrApplyStaleStatus);
  await runAppSessionsTest(testReconnectBackoffWakeSignalsReuseExistingLoop);
  await runAppSessionsTest(testLoadServerSessionStateUsesExplicitResultContract);
  await runAppSessionsTest(testSessionStatePollRetriesAfterTransientFailure);
  await runAppSessionsTest(testSessionStatePollTreats401AsAuthFailure);
  await runAppSessionsTest(testSessionStatePollDoesNotRetryAfterSessionSwitch);
  await runAppSessionsTest(testLateActiveRunSyncDoesNotMarkDraftStreaming);
  await runAppSessionsTest(testAddMenuAttachOptionTriggersFileInput);
  await runAppSessionsTest(testAddMenuLocationAddsReviewableDraft);
  await runAppSessionsTest(testLocationSharingCanBeDisabled);
  await runAppSessionsTest(testGoalStateResponseUpdatesChip);
  await runAppSessionsTest(testGoalModalSavesGoal);
  await runAppSessionsTest(testOpenMCPFromDraftCreatesSessionBeforeFetch);
  await runAppSessionsTest(testMCPStateResponseUpdatesHeaderPill);
  await runAppSessionsTest(testMCPHeaderPillOpensServersModal);
  await runAppSessionsTest(testMCPControlChangePatchesImmediately);
  await runAppSessionsTest(testMCPPatchConflictDoesNotOptimisticallyEnable);
