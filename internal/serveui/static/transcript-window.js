(function initTranscriptWindow(root, factory) {
  ...
  const DEFAULT_TRANSCRIPT_BUDGETS = Object.freeze({ maxMaterializedTurns: 60, overscanTurns: 8, maxRecentSkeletons: 2 });
  const TRANSCRIPT_BUDGETS = Object.freeze(Object.assign({}, DEFAULT_TRANSCRIPT_BUDGETS, root.TERM_LLM_TRANSCRIPT_BUDGETS || {}));
  ...
  class TranscriptWindow {
    constructor(sessionId, budgets = {}) {
      ...
      windowsBySession.set(this.sessionId, this);
    }
    ...
    applyIndex(envelope, etag = '') {
      ...
      const sparseClientMessageIDs = rows.client_message_ids && typeof rows.client_message_ids === 'object'
        ? rows.client_message_ids
        : {};
      const roles = String(rows.roles || '');
      const clientMessageIDs = ids.map((_id, ordinal) => String(sparseClientMessageIDs[String(ordinal)] || '').trim());
      ...
      const clientIdentitySet = new Set();
      for (const [key, value] of Object.entries(sparseClientMessageIDs)) {
        const ordinal = Number(key);
        const clientMessageId = String(value || '').trim();
        if (!Number.isInteger(ordinal) || ordinal < 0 || ordinal >= ids.length || roles[ordinal] !== 'u') {
          throw new Error('invalid sparse client_message_id index');
        }
        if (clientMessageId && clientIdentitySet.has(clientMessageId)) throw new Error('duplicate client_message_id in transcript index');
        if (clientMessageId) clientIdentitySet.add(clientMessageId);
      }
      ...
      while (divergence < common
        ...
        && this.clientMessageIDs[divergence] === clientMessageIDs[divergence]
      ...
      this.roles = roles;
      this.flags = flags;
      this.clientMessageIDs = clientMessageIDs;
      ...
      this.enforceBudget();
      return { changed, appendOnly, divergence, rollback, compactionChanged };
    }
    ...
    materialize(messages, options = {}) {
      ...
      for (const entry of messages) {
        ...
        if (ordinal >= 0) {
          if (!entry.responseId && this.responseIDs[ordinal]) entry.responseId = this.responseIDs[ordinal];
          if (!entry.clientMessageId && this.clientMessageIDs[ordinal]) entry.clientMessageId = this.clientMessageIDs[ordinal];
          if (entry.role === 'assistant' && this.assistantSegmentOrdinals[ordinal] >= 0) {
            entry.assistantSegmentOrdinal = this.assistantSegmentOrdinals[ordinal];
          }
        }
    ...
    rekey(sessionId) {
      ...
      if (windowsBySession.get(this.sessionId) === this) windowsBySession.delete(this.sessionId);
      this.sessionId = next;
      windowsBySession.set(this.sessionId, this);
      return this;
    }
    ...
    _checkInvariants() {
      ...
      if (this.seqs.length !== length || this.flags.length !== length || this.roles.length !== length
        || this.clientMessageIDs.length !== length || this.responseIDs.length !== length || this.assistantSegmentOrdinals.length !== length) {
  ...
  return Object.freeze({ TranscriptWindow, TRANSCRIPT_FLAG_COMPACTION_TAIL, TRANSCRIPT_FLAG_EMPTY_BODY, TRANSCRIPT_BUDGETS });
});
