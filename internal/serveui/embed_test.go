...
func TestSlashCommandsJS(t *testing.T) {
	node, err := exec.LookPath("node")
	if err != nil {
		t.Skip("node not found in PATH, skipping JS slash-command tests")
	}
	...
	script := filepath.Join(filepath.Dir(thisFile), "static", "slash_commands_test.js")
	if _, err := os.Stat(script); err != nil {
		t.Fatalf("slash_commands_test.js not found at %s: %v", script, err)
	}
