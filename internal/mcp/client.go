	...
	if c.automaticOAuthEnabled() {
		...
		if oauthConfig != nil {
			...
			if strings.TrimSpace(oauthConfig.ClientSecret) != "" {
				secret, err := config.ResolveDeferred(oauthConfig.ClientSecret)
				if err != nil {
					return nil, fmt.Errorf("mcp server %s: oauth.client_secret: %w", c.name, err)
				}
				options.ClientSecret = strings.TrimSpace(secret)
				if options.ClientSecret == "" {
					return nil, fmt.Errorf("mcp server %s: oauth.client_secret resolved to an empty value", c.name)
				}
			} else if oauthConfig.ClientSecretEnv != "" {
