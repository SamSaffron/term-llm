...
// secrets are referenced through the environment or a deferred value
// (op://, file://, $()) and are never stored literally in mcp.json.
type OAuthConfig struct {
	ClientID string `json:"client_id,omitempty"`
	// ClientSecret supports deferred resolution (op://, file://, $(), ${VAR})
	// and is resolved only when the server is connected.
