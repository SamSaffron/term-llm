...
func TestGatewayWireRequestRoundTripAndOmitsLocalState(t *testing.T) {
	req := Request{
		...
		Messages: []Message{
			...
			{Role: RoleAssistant, Parts: []Part{
				{Type: PartToolCall, ToolCall: &ToolCall{ID: "c1", Name: "view", Arguments: json.RawMessage(`{"x":1}`), Caller: "programmatic", ThoughtSig: []byte("sig")}},
				{Type: PartProviderReplay, ProviderReplay: &ProviderReplayItem{Raw: json.RawMessage(`{"type":"opaque"}`)}},
			}},
			{Role: RoleTool, Parts: []Part{{Type: PartToolResult, ToolResult: &ToolResult{ID: "c1", Name: "view", Content: "ok", ContentParts: []ToolContentPart{{Type: ToolContentPartText, Text: "ok"}, {Type: ToolContentPartImageData, ImageData: &ToolImageData{MediaType: "image/png", Base64: "aQ=="}}}, Images: []string{"/private/result.png"}}}}},
		},
	...
	text := string(wire)
	for _, forbidden := range []string{"WorkingDir", "working_dir", "/satellite/private", "/private/image.png", "/private/file.pdf", "/private/result.png", "private approval", "AllowedTools", "DebugRaw"} {
