...
func (m *Model) buildFooterLayout() footerLayout {
	...
	shellComposer := m.directShellComposerActive()
	shellAccent := shellComposer || m.directShellRun != nil
	if m.directShellRun != nil {
	...
	if m.textarea.Prompt != composerPrompt {
		m.textarea.Prompt = composerPrompt
		m.textarea.SetPromptFunc(lipgloss.Width(composerPrompt), func(textarea.PromptInfo) string {
...
func (m *Model) renderDirectShellInline() string {
	...
	if output == "" {
		...
		for _, line := range strings.Split(ansi.Hardwrap(output, width, true), "\n") {
...
func visibleDirectShellMessages(messages []session.Message) []session.Message {
	...
	for i := range messages {
		...
		display, ok := directShellVisibleResult(messages[i].TextContent)
		if !ok {
			continue
		}
		if visible == nil {
			visible = append([]session.Message(nil), messages...)
		}
		visible[i].TextContent = display
		visible[i].Parts = []llm.Part{{Type: llm.PartText, Text: display}}
	}
	if visible != nil {
		return visible
	}
...
func (m *Model) renderHistory() string {
	...
	messages = visibleDirectShellMessages(messages)
