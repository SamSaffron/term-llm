...
func (m *Model) Update(msg tea.Msg) (model tea.Model, cmd tea.Cmd) {
	...
	if updated, dispatchCmd, handled := m.dispatchTerminalUpdate(msg, &cmds); handled {
		if updated != nil {
			return updated, dispatchCmd
		}
	} else if updated, dispatchCmd, handled := m.dispatchFeatureUpdate(msg, &cmds); handled {
		if updated != nil {
			return updated, dispatchCmd
		}
	} else if updated, dispatchCmd, handled := m.dispatchConversationUpdate(msg, &cmds, &flushCmds); handled {
		if updated != nil {
			return updated, dispatchCmd
		}
	}
