...
func TestRawDuplicateMentionsAttachOnceWithoutCanonicalDeduplication(t *testing.T) {
	...
	if strings.Contains(formatted, "The following @-mentioned resources") || strings.Contains(formatted, "Contents of @") {
		t.Fatalf("formatted mention context contains redundant prose: %q", formatted)
	}
	if visible := llm.StripEmbeddedFileText("prompt" + formatted); visible != "prompt" {
		t.Fatalf("formatted mention context leaked into visible text: %q", visible)
	}
	if strings.Count(input, "@manual.txt") != 3 {
