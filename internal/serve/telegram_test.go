...
func TestTelegramAssistantTextCapturedIgnoresToolOnlyAssistant(t *testing.T) {
	messages := []llm.Message{
		...
		{
			Role:  llm.RoleTool,
			Parts: []llm.Part{{Type: llm.PartToolResult, ToolResult: &llm.ToolResult{ID: "call_1", Content: "ok"}}},
		},
	}
	if telegramAssistantTextCaptured(messages, "partial visible answer") {
		t.Fatal("tool-only assistant message should not count as captured visible partial text")
	}

	messages = append(messages, llm.AssistantText("partial visible answer"))
	if !telegramAssistantTextCaptured(messages, "partial visible answer") {
		t.Fatal("assistant text matching visible partial should count as captured")
	}
}
...
func TestStreamReply_PersistsErroredPartialAssistantReply(t *testing.T) {
	...
	if assistantPartialCount != 1 {
		t.Fatalf("persisted partial assistant count = %d, want 1; msgs=%#v", assistantPartialCount, msgs)
	}
	...
	for _, msg := range sess.history {
		if msg.Role == llm.RoleAssistant && telegramMessageVisibleText(msg) == "partial answer" {
			assistantHistoryCount++
		}
	}
	if assistantHistoryCount != 1 {
		t.Fatalf("history partial assistant count = %d, want 1; history=%#v", assistantHistoryCount, sess.history)
	}
}
...
func TestStreamReply_UncooperativeCleanupIsBounded(t *testing.T) {
	...
	incoming := tests[0]
	incoming.name = "incoming message replaces quarantined runtime"
	incoming.incoming = true
	tests = append(tests, incoming)
	blockedRecv := tests[0]
	blockedRecv.name = "Recv ignores watchdog cancellation"
	blockedRecv.watchdog = true
	blockedRecv.newMgr = func(started, release, stopped chan struct{}) *telegramSessionMgr {
		mgr := tests[0].newMgr(started, release, stopped)
		provider := &uncooperativeCloseProvider{started: started, release: release, stopped: stopped, blockRecv: true}
		mgr.streamEventTimeout = 15 * time.Millisecond
		mgr.settings.NewSession = func(context.Context) (*SessionRuntime, error) {
			return &SessionRuntime{Engine: llm.NewEngine(provider, llm.NewToolRegistry())}, nil
		}
		return mgr
	}
	tests = append(tests, blockedRecv)
...
func (r *telegramCompactionRunner) Run(ctx context.Context, req runpkg.Request, sink runpkg.EventSink) (runpkg.Result, error) {
	...
	if r.suspendFirst && len(r.requests) == 1 {
		return runpkg.Result{}, &llm.SuspendedError{Continuation: &llm.Continuation{Request: llm.Request{Messages: []llm.Message{
			llm.SystemText("be helpful"), llm.UserText("[Context Compaction] compact summary"), llm.AssistantText("retained tail"), llm.AssistantText(text),
		}}}}
	}
	return runpkg.Result{}, nil
}
...
func TestStreamReply_RetainsCompactedContextAndFullTranscript(t *testing.T) {
	for _, tc := range []struct {
		...
		suspendFirst    bool
	}{{}, {failFirst: true}, {failPersistence: true}, {failFirst: true, failPersistence: true}, {suspendFirst: true}, {suspendFirst: true, failPersistence: true}} {
		t.Run(fmt.Sprintf("error=%v/persistence_failure=%v/suspend=%v", tc.failFirst, tc.failPersistence, tc.suspendFirst), func(t *testing.T) {
			h := testutil.NewEngineHarness()
			...
			defer store.Close()
			runner := &telegramCompactionRunner{failFirst: tc.failFirst, suspendFirst: tc.suspendFirst}
			...
			// Round-trip the reload state, retaining the active projection separately
			// from the transcript, and restore through the real session owner.
			encoded, err := json.Marshal(telegramReloadChat{ChatID: 42, Meta: sess.meta, History: sess.history, ActiveHistory: sess.activeHistory, PromptPersisted: sess.systemPromptPersisted})
			if err != nil {
				t.Fatal(err)
			}
			var saved telegramReloadChat
			...
			for i, input := range []string{"second input", "third input"} {
				if tc.suspendFirst && i == 0 {
					if err := mgr.streamReplyContinuation(ctx, bot, sess, 42, llm.Message{}, nil, mgr.pendingReload[42]); err != nil {
						t.Fatal(err)
					}
				} else if err := mgr.streamReply(ctx, bot, sess, 42, llm.UserText(input)); err != nil {
					t.Fatal(err)
				}
			}
			...
			for i, req := range runner.requests[1:] {
				...
				for _, want := range []string{"be helpful", "[Context Compaction] compact summary", "retained tail", "second input"} {
					if tc.suspendFirst && want == "second input" {
						continue
					}
