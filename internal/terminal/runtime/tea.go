...
type View struct {
	...
	// PostFrame contains immutable side-band bytes that are written immediately
	// after this view reaches the renderer's flush boundary. Go strings can carry
	// arbitrary bytes, including terminal escape sequences. PostFrame is not part
	// of view equality: an unchanged view can still emit a payload.
	//
	// The renderer owns the value only when the view is accepted at a successful
	// flush boundary. If a newer view replaces this one first, the older payload
	// is discarded without copying. A payload is attempted at most once and only
	// after the frame write succeeds. A write error does not fail or roll back the
	// renderer frame.
	PostFrame string

	// PostFrameMsg builds the message delivered to Update after PostFrame is
	// attempted, with nil on success or the write error on failure. It belongs to
	// this exact View: replacement discards it together with PostFrame. Bubble Tea
	// schedules the returned message itself; this function must only construct and
	// return a message and must not call Program.Send. It runs after the renderer
	...
	// the renderer closes, including immediate shutdown caused by interruption,
	// cancellation, Kill, or panic. Cleanup is emitted before alternate-screen
	// exit and other terminal mode restoration. Use this only for idempotent
	// cleanup of terminal resources owned by the current View.
...
func (p *Program) eventLoop(model Model, cmds chan Cmd) (Model, error) {
	rendererMsgs := p.renderer.messages()
	for {
		var msg Msg
		// Renderer results are part of the accepted-frame transaction. Prefer a
		// queued result before beginning another external event-loop turn, which
		// keeps the one-slot renderer queue bounded even under sustained input.
		select {
		case msg = <-rendererMsgs:
		default:
			select {
			case <-p.ctx.Done():
				return model, nil
...
func (p *Program) Run() (returnModel Model, returnErr error) {
	...
	// Set the detected color profile on the renderer.
...
func (p *Program) flush() error {
	...
	want := p.outputBuf.Len()
	n, err := p.output.Write(p.outputBuf.Bytes())
	p.outputBuf.Reset()
	if err != nil {
		if p.renderer != nil {
	...
	if n != want {
		...
		return fmt.Errorf("error writing to output: %w", io.ErrShortWrite)
	}
	return nil
}
