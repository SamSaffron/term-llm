...
func withTestColorProfile(profile colorprofile.Profile) ProgramOption {
	return func(p *Program) { p.profile = &profile }
}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			p := NewProgram(m,
				...
				withTestColorProfile(colorprofile.ANSI256),
...
func TestClearMsg(t *testing.T) {
	...
	tests := []test{
		...
		},
		{
			...
			cmds: []Cmd{RequestBackgroundColor},
		},
	}
