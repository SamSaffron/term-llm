...
func TestRendererOutput(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			s.SetScrollOptim(runtime.GOOS != "windows") // Disable scroll optimization on Windows for consistent results
...
type shortWriteWithoutError struct{}

func (shortWriteWithoutError) Write(p []byte) (int, error) {
	return len(p) / 2, nil
}

func TestTerminalRendererFlushReportsShortWrite(t *testing.T) {
	r := NewTerminalRenderer(shortWriteWithoutError{}, []string{"TERM=xterm-256color"})
	_, _ = r.WriteString("terminal payload")
