...
func TestClassifyConfigShowResetAndCompletion(t *testing.T) {
	...
	for _, reset := range []bool{false, true} {
		...
		// Alias api_key specs must inherit the sensitive flag, so the credential
		// is redacted in config show exactly like the built-in provider's.
		if strings.Contains(out.String(), "exit 1") {
			t.Fatalf("alias credential leaked into config show (reset=%t):\n%s", reset, out.String())
		}
		if !reset {
			custom, _ := providers["custom"].(map[string]any)
			if custom["api_key"] != "<redacted>" {
				t.Fatalf("alias api_key = %v, want <redacted>", custom["api_key"])
			}
		}
