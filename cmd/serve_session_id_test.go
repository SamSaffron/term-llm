...
func TestServeSessionIDLookupFailureIsServerError(t *testing.T) {
	store, err := session.NewSQLiteStore(session.Config{Enabled: true, Path: filepath.Join(t.TempDir(), "sessions.db")})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, endpoint := range []struct {
		...
		t.Run(endpoint.path, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodPost, endpoint.path, strings.NewReader(endpoint.body))
			req.Header.Set("Content-Type", "application/json")
			req.Header.Set("X-Term-LLM-Session-ID", "chat/session")
			rr := httptest.NewRecorder()
			endpoint.handle(srv, rr, req)
			if rr.Code != http.StatusInternalServerError || strings.Contains(rr.Body.String(), "database") {
				t.Fatalf("status/body = %d %s", rr.Code, rr.Body.String())
			}
		})
	}
