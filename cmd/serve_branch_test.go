...
func TestResponsesBranchCreatesChildAndContinuesWithoutChangingSource(t *testing.T) {
	...
	provider := llm.NewMockProvider("mock").
		AddTurn(llm.MockTurn{Text: "- The second attempt discovered a parser edge case.", Usage: llm.Usage{InputTokens: 19, OutputTokens: 7}}).
		AddTextResponse("alternate answer")
	...
	defer manager.Close()
	if _, err := manager.GetOrCreate(ctx, sourceID); err != nil {
		t.Fatal(err)
	}
	srv := &serveServer{sessionMgr: manager, store: store}
	body := fmt.Sprintf(`{"input":"alternate question","stream":true,"previous_response_id":%q,"branch":true,"branch_context":{"mode":"notes"},"expected_rev":%d,"idempotency_key":"branch-request"}`, anchorID, state.Rev)
	...
	updatedSource, err := store.Get(ctx, sourceID)
	if err != nil || updatedSource.InputTokens != 19 || updatedSource.OutputTokens != 7 {
		t.Fatalf("path-note helper usage not attributed to source: session=%#v err=%v", updatedSource, err)
	}
	childMessages, _ := store.GetMessages(ctx, childID, 0, 0)
	if len(childMessages) != 5 {
	...
	for i := range want {
		if i == 2 {
			if _, ok := childMessages[i].PathNoteProvenance(); !ok || !strings.Contains(childMessages[i].PathNoteDisplayText(), "parser edge case") {
				t.Fatalf("child path note = %#v", childMessages[i])
			}
			continue
		}
	...
	if len(provider.Requests) != 2 {
		t.Fatalf("provider requests = %d, want notes + response", len(provider.Requests))
	}
	providerTexts := make([]string, 0, len(provider.Requests[1].Messages))
	...
	joinedProviderText := strings.Join(providerTexts, "|")
	for _, wantText := range []string{"first", "first answer", "[Path Notes]", "parser edge case", "alternate question"} {
		if !strings.Contains(joinedProviderText, wantText) {
			t.Fatalf("provider copied prefix/path notes/input missing %q: %q", wantText, joinedProviderText)
		}
	}
	...
	if len(provider.Requests) != 2 {
		t.Fatalf("provider requests after retry = %d, want 2", len(provider.Requests))
	}
	updatedSource, _ = store.Get(ctx, sourceID)
	if updatedSource.InputTokens != 19 || updatedSource.OutputTokens != 7 {
		t.Fatalf("retry duplicated helper usage: %#v", updatedSource)
	}
...
func TestPrepareBranchPathNoteOnceSurvivesCancellationAndDeduplicates(t *testing.T) {
	ctx := context.Background()
	store, err := session.NewStore(session.Config{Enabled: true, Path: filepath.Join(t.TempDir(), "sessions.db")})
	if err != nil {
		t.Fatal(err)
	...
	defer store.Close()
	const sourceID = "deduplicated-path-notes"
	if err := store.Create(ctx, &session.Session{ID: sourceID, Provider: "mock", ProviderKey: "mock", Model: "mock-model"}); err != nil {
		t.Fatal(err)
	}
	if err := store.ReplaceMessages(ctx, sourceID, []session.Message{
		*session.NewMessage(sourceID, llm.UserText("anchor"), -1),
		*session.NewMessage(sourceID, llm.AssistantText("important later finding"), -1),
	}); err != nil {
		t.Fatal(err)
	}
	messages, _ := store.GetMessages(ctx, sourceID, 0, 0)
	provider := llm.NewMockProvider("mock").AddTurn(llm.MockTurn{
		Text: "- Keep the later finding.", Delay: 100 * time.Millisecond,
		Usage: llm.Usage{InputTokens: 11, OutputTokens: 4},
	})
	srv := &serveServer{
		store: store,
		pathNotesProviderFactory: func(_, _ string) (llm.Provider, error) {
			return provider, nil
		},
	}
	type result struct {
	...
	call := func(callCtx context.Context) result {
		note, status, message, _, release := srv.prepareBranchPathNoteOnce(callCtx, sourceID, messages[0].ID, "same-request", &responsesBranchContextRequest{Mode: "notes"})
		return result{note: note, status: status, message: message, release: release}
	}

	requestCtx, cancelRequest := context.WithCancel(ctx)
	ownerDone := make(chan result, 1)
	go func() { ownerDone <- call(requestCtx) }()
	waitForServeCondition(t, time.Second, func() bool { return provider.CurrentTurn() == 1 }, "path-note helper to start")
	_, mismatchStatus, _, _, mismatchRelease := srv.prepareBranchPathNoteOnce(ctx, sourceID, messages[0].ID, "same-request", &responsesBranchContextRequest{Mode: "focused", Focus: "different"})
	mismatchRelease()
	if mismatchStatus != http.StatusConflict {
		t.Fatalf("mismatched in-flight request status = %d, want conflict", mismatchStatus)
	}
	cancelRequest()
	waiter := call(ctx)
	owner := <-ownerDone
	...
	if owner.status != 0 || waiter.status != 0 || owner.note == nil || waiter.note == nil || owner.note.Text != waiter.note.Text {
		t.Fatalf("shared results owner=%#v waiter=%#v", owner, waiter)
	}
	if got := len(provider.RecordedRequests()); got != 1 {
		t.Fatalf("provider requests = %d, want one shared helper call", got)
	}
	updated, err := store.Get(ctx, sourceID)
	if err != nil || updated.InputTokens != 11 || updated.OutputTokens != 4 {
		t.Fatalf("detached helper usage = %#v, %v", updated, err)
	}
}
...
func TestSessionMessageEntriesExposeOnlyMarkedDeveloperPathNotes(t *testing.T) {
	marked := session.NewPathNoteMessage("child", "- Useful finding.", llm.PathNoteProvenance{SourceSessionID: "source", AnchorMessageID: 4}, 2)
	marked.ID = 9
	entries := (&serveServer{}).sessionMessageEntries([]session.Message{
		*session.NewMessage("child", llm.Message{Role: llm.RoleDeveloper, Parts: []llm.Part{{Type: llm.PartText, Text: "secret platform prompt"}}}, 1),
		*marked,
	})
	if len(entries) != 1 || entries[0].Role != "path-note" || len(entries[0].Parts) != 1 {
		t.Fatalf("entries = %#v", entries)
	}
	part := entries[0].Parts[0]
	if part.Type != "path_note" || part.Text != "- Useful finding." || part.PathNote == nil || part.PathNote.SourceSessionID != "source" {
		t.Fatalf("path note part = %#v", part)
	}
}
