...
func addClassifyConnectionOutputFlags(cmd *cobra.Command, opts *classifyOptions, valueFormat bool) {
	...
	_ = cmd.RegisterFlagCompletionFunc("provider", classifyProviderCompletion)
	if valueFormat {
...
func runClassifyModels(cmd *cobra.Command, opts *classifyOptions, deps classifyDeps) error {
	...
	out, err := formatClassifyModels(resp, opts.format)
	if err != nil {
		return err
	}
	return writeClassifyOutput(cmd, opts.output, out)
...
func formatClassifyResponse(resp *typesafe.Response, opts *classifyOptions) ([]byte, error) {
	switch opts.format {
	case "json":
		return formatClassifyJSON(resp.Raw, resp)
	case "table":
...
func formatClassifyModels(resp *typesafe.ModelsResponse, format string) ([]byte, error) {
	switch format {
	case "json":
		return formatClassifyJSON(resp.Raw, resp)
	case "table":
...
func formatClassifyJSON(raw json.RawMessage, fallback any) ([]byte, error) {
	if len(raw) > 0 {
