...
func (s *serveServer) publishResponseRunEvent(run *responseRun, event string, payload map[string]any) {
	...
	case "response.created",
		...
		"response.steering", "response.steering.queued", "response.compaction", "response.model_switch":
	default:
		return
	}
	...
	case "response.file_change", "response.filesystem_observation", "response.output_claim_diagnostic":
		...
		s.publishEvent(base)
	case "response.steering", "response.steering.queued", "response.compaction", "response.model_switch":
