...
var errUnsafeSessionID = errors.New("new session ID must not contain path separators or control characters, or be . or ..")

func sessionIDErrorResponse(err error) (int, string, string) {
	if errors.Is(err, errUnsafeSessionID) {
		return http.StatusBadRequest, "invalid_request_error", err.Error()
	}
	return http.StatusInternalServerError, "server_error", "failed to look up session ID"
}
