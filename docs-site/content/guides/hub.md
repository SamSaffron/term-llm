...
## Hub-wide activity and attention

Nodes that advertise attention protocol v2 publish complete paginated snapshots of durable running runs, level-triggered input-required runs, and terminal conversations that have not yet been visited. Protocol v1 nodes continue to contribute running and terminal-unseen state. Hub collects these snapshots in the background over the same direct or reverse transport used for node requests and stores a private projection in `<data-dir>/hub/attention.db` (mode 0600). Collection is faster while cached work is active or waiting, slower while idle, and uses bounded jitter/backoff after failures. The projection contains titles, counts, broad interaction kinds, and lifecycle metadata only—never questions, approval targets, prompts, transcripts, response bodies, backend URLs, or node tokens.

The dashboard's **Needs your input** section links to conversations blocked on `ask_user`, shell/file approval, workspace access, or another actionable approval. These rows remain solid green until the underlying interaction resolves; visiting or dismissing the modal is not an acknowledgement. Stale cached rows are explicitly shown as last-known state rather than current actionable work.

The separate **Ready to review** section combines unseen completed, failed, reviewable-cancelled, and orphan-recovery conversations from every node. `GET /api/attention` exposes the same privacy-safe global/per-node counts plus bounded input-required and newest-first terminal lists; its `limit` is 1–500 (default 200) and `has_more` reports truncation. Conversation links navigate through the normal node proxy. Clicking a terminal row is not itself an acknowledgement: the proxied node web app clears the marker only after the route is selected, the page is visible, and transcript bodies are installed through the marker's final revision. Hub learns that authoritative change on a later collection pass.

Hub retains its last successful rows when a node is offline, times out, returns malformed pagination, or loses a previously advertised capability. It replaces one node's projection only after every protocol-supported kind has the same store instance and snapshot version. If a node database is reset, the new stable store instance ID prevents unrelated session IDs from being merged. Nodes that never supported the API report attention as unavailable rather than falsely reporting zero.

Conversation rows preserve the visual priority: solid green for input required, pulsing green for running, and solid green for terminal unseen. A session that is both running and blocked is rendered once as input required; the underlying projections remain separate so resolving the prompt reveals running state without clearing terminal seen watermarks.

The Hub dashboard's solid green node indicator means at least one child conversation is running, input-required, or terminal-unseen. The web agents sidebar reserves its solid green dot for terminal-unseen work on other agents that needs attention; running-only and input-required-only agents stay quiet there. Hub aggregate indicators never pulse.
