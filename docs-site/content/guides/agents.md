...
- `@extension-builder` to [make the web interface yours](/guides/extensions/) with CSS and JavaScript
...
The built-in `sysadmin` agent uses `workspace: none`: its execution directory remains available to the shell, but it neither proposes that directory as a primary workspace nor exposes `manage_workspace`. This opt-out does not revoke separate read, write, shell, session, project, Guardian, or yolo authority. Its `read.dirs: ["/"]` grant is intentionally sensitive and authorizes host-wide file-tool reads subject to operating-system access and tool traversal limits. Because those reads need no approval, the agent ships with `search: false`: `web_search` and `read_url` have no approval gate, and together they would let injected instructions exfiltrate host data without a prompt. Enable search for a session only when you need it. `printenv` is not pre-approved for the same reason.
