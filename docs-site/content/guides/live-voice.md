...
Requests longer than 40 words skip classification as a cost and latency exception, not as a safety rule. The live classifier uses a three-second default deadline; an explicitly configured non-default `classify.providers.<name>.timeout_seconds` remains authoritative. A navigation request that also asks for work—“in the album chat, make the ornaments smaller”—also passes through in full: Phase 1 does not split it, extract a remainder, or promise navigate-then-continue delivery. The classify backend does not stop active work, answer private side questions, or change voices. Use the `agent` backend for current-call voice control through `live_settings`.

`live.classify.shadow: true` is the default. Shadow still calls the classifier and therefore adds provider latency, occupies the serial routing worker, and can encounter its four-entry backpressure limit, but action selection remains the same as `off`: every label becomes ordinary work or guidance. It constructs no switch resolver, installs no mutating control authority, and does not advertise navigation handling to the voice model.

With shadow disabled, a confident status request is answered from running/recent sessions and jobs. A confident new-session request starts an empty conversation with the call's current defaults. A switch request gets a second, restricted resolver turn with only `session_directory` and `live_switch_session`; successful switches, host refusals, and ambiguous directory results are spoken, while invalid or unauthorized calls and prose-only claims prove nothing and fail open.

**Classification is fail-open.** Provider errors, timeouts, malformed or unknown answers, missing probabilities, below-threshold labels, action failures, and inconclusive switch resolution all send the original input to the session lane. A missing selected classify provider or credential is different: it is a startup configuration error, while an outage after startup fails open per request.

When `live.classify.log_decisions` is enabled, `term-llm live decisions [--since 24h]` reads the `route_decisions` audit rows. They include labels, probabilities, action/resolver outcomes, latency, and errors. With `live.classify.log_state: true`, they also include bounded speech and session titles. These rows live in the live diagnostics database and follow its retention/deletion lifecycle. “Not in the chat transcript” does not mean “not stored”; disable decision logging entirely, or disable state logging while retaining aggregate routing evidence, when that persistence is inappropriate.
...
| Route | Purpose |
| --- | --- |
| `POST /v1/live/sessions` | Start a call. WebRTC providers exchange an SDP offer and answer; Gemini returns an HTTP PCM capability, or the legacy direct WebSocket URL when explicitly requested. Optional `delegation_mode` and `delegation_context` choose who executes delegations; the response echoes the effective `delegation_mode`. |
...
| `POST /v1/live/sessions/{live_id}/delegations/{delegation_id}/result` | Return one client-executed delegation result. Client delegation mode only. |
| `GET /v1/live/sessions/{live_id}/events` | Stream `live.started`, `live.transcript`, `live.delegation`, `live.delegation_requested`, `live.interrupted`, `live.error`, `live.session_changed`, and `live.ended` over SSE, with `?after=` replay. |
...
Only the call's transcript, delegation state, and—in client delegation mode—the delegated request text travel on the live event stream. Delegated turns reach the browser through the normal response-run path, so an open call does not flood the shared server-event feed.

## Client-owned delegation

By default the host executes every delegation as a chat turn in the bound session. An authenticated client—typically a phone app with device-native tools the server cannot reach—can instead own execution by starting the call with `{"delegation_mode":"client"}`. Omitting the field, or sending `"server"`, keeps the default. Any other value is rejected with `400`; an unknown mode is never quietly defaulted. The start response echoes the effective `delegation_mode`, and `/v1/capabilities` advertises `"delegation_modes":["server","client"]` so a client can request the mode only where it exists. A client that sees no `delegation_modes` field should fall back to server mode, where its device-native tools are unavailable during a call.

`delegation_context` is the optional plain-language description of what the device can do, for example “The device can control Apple Music: play, pause, skip, search, and manage playlists.” Without it the voice model answers “play some jazz” by saying it cannot play music, because nothing else in its context describes device tools. It is at most 2 KiB of trimmed plain text—every control character except `\n` is rejected—and it is accepted **only** with `"delegation_mode":"client"`, since in server mode it would make the host promise capabilities no delegation can reach. It is never a tool schema and is never forwarded to `/v1/responses`: tool authority stays on the device, which declares its real tools per request. term-llm collapses it to a single quoted line inside a fixed host preamble that names it as reported data rather than instructions, and never persists it to the chat session.

In client mode the delegation lifecycle is unchanged and still host-owned. Between `running` and the terminal state, term-llm publishes `live.delegation_requested` with `{"session_id","delegation_id","input","resend"}`. The event travels the same ring buffer as every other live event, so `?after=` replay works, and because that ring also carries every interim transcript, a single publication is not a guaranteed delivery: while a delegation is pending the server republishes the same `delegation_id` and `input` every 15 seconds with `"resend":true`. **Clients must deduplicate by `delegation_id`.**

The client executes `input` through its own `/v1/responses` flow bound to the event's `session_id`, then posts exactly one of `output` or `error` to `POST /v1/live/sessions/{live_id}/delegations/{delegation_id}/result`. An empty or whitespace-only `output` is a failure, not a silent success, and is rejected with `400`; so is trailing data after the JSON object. The body is bounded at 64 KiB and a larger one is rejected with `413`, because spoken output is truncated to its head and tail before the voice model ever sees it; `error` text is additionally capped at 1 KiB, since it is published verbatim as the delegation's failure text. An optional `response_id` is correlation metadata that is logged and never trusted, and is ignored when a replay is compared. An optional `session_id` is a consistency check against the binding the server captured when it published the request—never authority. The first accepted result returns `200 {"ok":true}`; an identical replay returns `200 {"ok":true,"duplicate":true}` so a client that lost its response can retry safely; a differing replay, a mismatched `session_id`, or a delegation nobody is waiting for any more returns `409`; an unknown `delegation_id`, or a `live_id` that has already been torn down, returns `404`. A client that receives `404` or `409` should stop: the server no longer wants that result.

Three caveats apply to this mode. Delegations run through the controller's serial lane, so there is at most **one outstanding request per call**; a second spoken request waits in the host queue and the voice model is not told about it. The session-busy check also stays on the host, so a delegation offered while a web turn already owns the session produces the same spoken busy notice and retry as server mode instead of a `409` the device discovers for itself—a turn that starts *after* the request was published is not covered, and the client sees that `409` from `/v1/responses` itself. And a client calling `/v1/responses` directly does not get the live execution context a host-run delegation gets, so its replies may be longer and less speakable than server-mode answers. A session switch affects only future delegations: a request already published stays bound to the session named in its event, and a switch that lands while a request is being published makes that request fail so the voice model can repeat it against the new binding.

Authorization is the serve bearer token, exactly as for `/text` and `/session`. `live_id` and `delegation_id` are correlation, not capabilities—delegation IDs come from the provider and are broadcast on the call's own SSE stream. A result is never accepted for a delegation this call did not request, one already answered differently, or a call that has ended. Every way a call can end—`DELETE`, the idle reaper, server shutdown, or the provider hanging up—releases its waiters immediately rather than holding them to the five-minute delegation timeout.
