...
On every classified delegation, term-llm **sends off-host to the selected classify provider** the delegation message, up to 4,096 runes of recent transcript delta, and up to 12 recent session titles (plus bounded active-run metadata). This also happens in shadow mode; shadow suppresses host actions, not the provider request.
