...
| `live.classify.provider`         | _(unset)_                                          | Classification provider key; inherits `classify.default_provider`. With `control_plane: classify`, an unresolvable provider or credential is a startup configuration error. |
...
| `live.classify.log_decisions`    | `true`                                             | Persist `route_decisions` audit rows for `term-llm live decisions [--since]`. |
| `live.classify.log_state`        | `true`                                             | Include bounded speech and session-title state in decision rows. `false` keeps probabilities, labels, outcomes, latency, and errors but omits state JSON. |
