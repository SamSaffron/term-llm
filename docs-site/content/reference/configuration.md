...
| `live.classify.shadow`           | `true`                                             | Classify and log but act as though every label were ordinary `steer`. This still adds classifier latency and routing-worker backpressure; it installs no control authority and advertises no navigation handling. |
...
Every classify delegation sends the selected classification provider the delegation message, up to 4,096 runes of transcript delta, up to 12 recent session titles, and bounded active-run metadata. Shadow mode still sends this request; it changes host action selection only.
