...
A local launch records its canonical runtime/project directory as the session's **proposed primary workspace**. Launch location alone grants no file authority. On the first file, search, or image path access inside it outside yolo, term-llm pauses before Guardian, static allowlists, or execution and asks the human once whether this session may read and write the entire canonical workspace. The only choices are **Allow this session's canonical workspace read/write** or **Deny**, and the prompt explicitly notes that shell commands remain separately controlled. An allow is persisted with SQLite-backed sessions and resume does not reprompt; a denial blocks the triggering call. Without a human confirmation transport, non-yolo access fails closed. Yolo bypasses confirmation without invoking the prompt or persisting workspace authority; if the session later leaves yolo while the primary is still unconfirmed, the next access asks normally.
