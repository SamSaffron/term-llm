...
A local launch records its canonical runtime/project directory as the session's **proposed primary workspace**. Launch location alone grants no file authority. On the first file, search, or image path access inside it outside yolo, term-llm pauses before Guardian, static allowlists, or execution and asks the human whether it may read and write the entire canonical workspace. The choices are **Allow this session's canonical workspace read/write**, **Allow and remember this canonical workspace**, or **Deny**; the prompt explicitly notes that shell commands remain separately controlled. A session-only allow is persisted with SQLite-backed sessions, so resume does not reprompt. A remembered allow records the exact canonical root in `$XDG_CONFIG_HOME/term-llm/remembered-workspaces.yaml` (falling back to `~/.config`) and automatically applies to future sessions that propose the same root. To revoke a remembered approval, remove that root's entry from this YAML file. A denial blocks the triggering call. Without a human confirmation transport or a matching remembered approval, non-yolo access fails closed. Yolo bypasses confirmation without invoking the prompt or persisting workspace authority; if the session later leaves yolo while the primary is still unconfirmed and unremembered, the next access asks normally.

Sibling directories are never included. Switching a session to another Git worktree replaces the proposal, invalidates a mismatched primary confirmation, and requires confirmation on first access to the new workspace without removing separately granted references.
...
Without an explicit `guardian.model`, Guardian prefers the selected provider's configured `fast_model` and switches to its `fast_provider` when configured, preserving the provider/model pair. It then falls back to that provider's normal model, the provider type's built-in fast default, or a compatible active model. This is a compatibility change: users who want Guardian to use the main/large model should pin `guardian.model`; an explicit model never triggers an implicit fast-provider switch.
